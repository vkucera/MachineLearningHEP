...
from support.utilities_plot import (
    Errors,
    calc_systematic_mesondoubleratio,
    calc_systematic_mesonratio,
    calc_systematic_multovermb,
    divide_all_by_first_multovermb,
    divide_by_eachother,
    divide_by_eachother_barlow,
    plot_histograms,
    save_histograms,
)
...
FILEMB_LC_ALLHISTOS = (
    "finalcrossLcpKpippMBvspt_ntrklmult0_WithoutNbx2_WithAllHistos.root"
)

ERROR_FILESLC = [
    "syst_QM19/LcpKpipp/errors_histoSigmaCorr_0.yaml",
...
ERROR_FILESD0 = [
    "syst_QM19/D0pp/MBvspt_ntrkl_Lcbinning/errors_histoSigmaCorr_0.yaml",
...
for mb in range(NMULTBINS):
    ...
    if mb != 4:
        if mb == 0:
            HISTOEFF = extract_histo(
                CASE, ANA_MB, mb, "hDirectEffpt", PATH_MIX, FILEMB_LC_ALLHISTOS
            )
        else:
    ...
    if mb == 4:
        GRFD = extract_histo(
            CASEAV, ANA_MB, mb, "gFcCorrConservative_average", PATH_MIX
        )
    ERRORNB = []
...
HISTO06099.Scale(1.0 / SIGMAV0)
HISTO06099.Scale(1.0 / BRLC)
...
TOT_MULT_OVER_MB = calc_systematic_multovermb(
    ERRS[1], ERRSFOR19[0], HISTOS_DIVMB[0].GetNbinsX(), True
)
TOT_MULT_OVER_MB_FD = calc_systematic_multovermb(
    ERRS[1], ERRSFOR19[0], HISTOS_DIVMB[0].GetNbinsX(), True, True
)
TOT_MULT_OVER_MB_WOFD = calc_systematic_multovermb(
    ERRS[1], ERRSFOR19[0], HISTOS_DIVMB[0].GetNbinsX(), True, False
)

ERRS_GR_DIV_TOT.append(
    Errors.make_root_asymm(HISTOS_DIVMB[0], TOT_MULT_OVER_MB, const_x_err=0.3)
)
ERRS_GR_DIV_WOFD.append(
    Errors.make_root_asymm(HISTOS_DIVMB[0], TOT_MULT_OVER_MB_WOFD, const_x_err=0.3)
)
ERRS_GR_DIV_FD.append(
    Errors.make_root_asymm(HISTOS_DIVMB[0], TOT_MULT_OVER_MB_FD, const_x_err=0.3)
)
...
TOT_MULT_OVER_MB = calc_systematic_multovermb(
    ERRS[2], ERRS[0], HISTOS[2].GetNbinsX(), True
)
TOT_MULT_OVER_MB_FD = calc_systematic_multovermb(
    ERRS[2], ERRS[0], HISTOS[2].GetNbinsX(), True, True
)
TOT_MULT_OVER_MB_WOFD = calc_systematic_multovermb(
    ERRS[2], ERRS[0], HISTOS[2].GetNbinsX(), True, False
)

ERRS_GR_DIV_TOT.append(
    Errors.make_root_asymm(HISTOS_DIVMB[1], TOT_MULT_OVER_MB, const_x_err=0.3)
)
ERRS_GR_DIV_WOFD.append(
    Errors.make_root_asymm(HISTOS_DIVMB[1], TOT_MULT_OVER_MB_WOFD, const_x_err=0.3)
)
ERRS_GR_DIV_FD.append(
    Errors.make_root_asymm(HISTOS_DIVMB[1], TOT_MULT_OVER_MB_FD, const_x_err=0.3)
)
...
TOT_MULT_OVER_MB = calc_systematic_multovermb(
    ERRS[3], ERRSFOR3059[0], HISTOS_DIVMB[2].GetNbinsX(), True
)
TOT_MULT_OVER_MB_FD = calc_systematic_multovermb(
    ERRS[3], ERRSFOR3059[0], HISTOS_DIVMB[2].GetNbinsX(), True, True
)
TOT_MULT_OVER_MB_WOFD = calc_systematic_multovermb(
    ERRS[3], ERRSFOR3059[0], HISTOS_DIVMB[2].GetNbinsX(), True, False
)

ERRS_GR_DIV_TOT.append(
    Errors.make_root_asymm(HISTOS_DIVMB[2], TOT_MULT_OVER_MB, const_x_err=0.3)
)
ERRS_GR_DIV_WOFD.append(
    Errors.make_root_asymm(HISTOS_DIVMB[2], TOT_MULT_OVER_MB_WOFD, const_x_err=0.3)
)
ERRS_GR_DIV_FD.append(
    Errors.make_root_asymm(HISTOS_DIVMB[2], TOT_MULT_OVER_MB_FD, const_x_err=0.3)
)
...
TOT_MULT_OVER_MB = calc_systematic_multovermb(
    ERRS[4], ERRS[0], HISTOS_DIVMB[3].GetNbinsX(), False
)
TOT_MULT_OVER_MB_FD = calc_systematic_multovermb(
    ERRS[4], ERRS[0], HISTOS_DIVMB[3].GetNbinsX(), False, True
)
TOT_MULT_OVER_MB_WOFD = calc_systematic_multovermb(
    ERRS[4], ERRS[0], HISTOS_DIVMB[3].GetNbinsX(), False, False
)

ERRS_GR_DIV_TOT.append(
    Errors.make_root_asymm(HISTOS_DIVMB[3], TOT_MULT_OVER_MB, const_x_err=0.3)
)
ERRS_GR_DIV_WOFD.append(
    Errors.make_root_asymm(HISTOS_DIVMB[3], TOT_MULT_OVER_MB_WOFD, const_x_err=0.3)
)
ERRS_GR_DIV_FD.append(
    Errors.make_root_asymm(HISTOS_DIVMB[3], TOT_MULT_OVER_MB_FD, const_x_err=0.3)
)
...
SAVE_PATH = make_standard_save_path(
    f"LcpKpiAvgMultOverMB_{ANA_MB}_19_1029_3059_6099", PATH_OUT
)
save_histograms(
...
for mb, _ in enumerate(HISTOS_LCOVERD0):
    tot_Lc_over_D0 = calc_systematic_mesonratio(
        ERRS_LC[mb], ERRS_D0[mb], HISTOS_LCOVERD0[mb].GetNbinsX()
    )
    tot_Lc_over_D0_FD = calc_systematic_mesonratio(
        ERRS_LC[mb], ERRS_D0[mb], HISTOS_LCOVERD0[mb].GetNbinsX(), True
    )
    tot_Lc_over_D0_WOFD = calc_systematic_mesonratio(
        ERRS_LC[mb], ERRS_D0[mb], HISTOS_LCOVERD0[mb].GetNbinsX(), False
    )
    ERRS_GR_DIVD0_TOT.append(
        Errors.make_root_asymm(HISTOS_LCOVERD0[mb], tot_Lc_over_D0, const_x_err=0.3)
    )
    ERRS_GR_DIVD0_FD.append(
        Errors.make_root_asymm(HISTOS_LCOVERD0[mb], tot_Lc_over_D0_FD, const_x_err=0.3)
    )
    ERRS_GR_DIVD0_WOFD.append(
        Errors.make_root_asymm(
            HISTOS_LCOVERD0[mb], tot_Lc_over_D0_WOFD, const_x_err=0.3
        )
    )
...
# Save globally in Lc directory
SAVE_PATH = make_standard_save_path(
    f"LcpKpiAvgOverD0_{ANA_MB}_1999_19_1029_3059_6099", PATH_OUT
)
save_histograms(
    [*HISTOS_LCOVERD0, *ERRS_GR_DIVD0_TOT, *ERRS_GR_DIVD0_FD, *ERRS_GR_DIVD0_WOFD],
    SAVE_PATH,
)
...
HISTO_DR_M.append(
    divide_by_eachother(
        [HISTOS_LCOVERD0[2]], [HISTOS_LCOVERD0[1]], None, [1, 2, 4, 6, 8, 12]
    )[0]
)
HISTO_DR_M.append(
    divide_by_eachother(
        [HISTOS_LCOVERD0[3]], [HISTOS_LCOVERD0[1]], None, [2, 4, 6, 8, 12]
    )[0]
)
HISTO_DR_M.append(
    divide_by_eachother(
        [HISTOS_LCOVERD0[4]], [HISTOS_LCOVERD0[1]], None, [1, 2, 4, 6, 8, 12]
    )[0]
)
...
for mb, _ in enumerate(HISTO_DR_M):
    ...
    if mb == 2:
        num = 4
    tot_Lc_over_D0_DR = calc_systematic_mesondoubleratio(
        ERRS_LC[num],
        ERRS_D0[num],
        ERRS_LC[den],
        ERRS_D0[den],
    ...
    tot_Lc_over_D0_DR_FD = calc_systematic_mesondoubleratio(
        ERRS_LC[num],
        ERRS_D0[num],
        ERRS_LC[den],
        ERRS_D0[den],
    ...
    tot_Lc_over_D0_DR_WOFD = calc_systematic_mesondoubleratio(
        ERRS_LC[num],
        ERRS_D0[num],
        ERRS_LC[den],
        ERRS_D0[den],
    ...
    ERRS_GR_DR_M_TOT.append(
        Errors.make_root_asymm(HISTO_DR_M[mb], tot_Lc_over_D0_DR, const_x_err=0.3)
    )
    ERRS_GR_DR_M_FD.append(
        Errors.make_root_asymm(HISTO_DR_M[mb], tot_Lc_over_D0_DR_FD, const_x_err=0.3)
    )
    ERRS_GR_DR_M_WOFD.append(
        Errors.make_root_asymm(HISTO_DR_M[mb], tot_Lc_over_D0_DR_WOFD, const_x_err=0.3)
    )
...
SAVE_PATH = make_standard_save_path(FILENAME, PATH_OUT)
save_histograms(
    [*HISTO_DR_M, *ERRS_GR_DR_M_TOT, *ERRS_GR_DR_M_FD, *ERRS_GR_DR_M_WOFD], SAVE_PATH
)
...
HISTO_DR_MB.append(
    divide_by_eachother_barlow(
        [HISTOS_LCOVERD0[1]], [HISTOS_LCOVERD0[0]], None, [1, 2, 4, 6, 8, 12]
    )[0]
)
HISTO_DR_MB.append(
    divide_by_eachother_barlow(
        [HISTOS_LCOVERD0[3]], [HISTOS_LCOVERD0[0]], None, [2, 4, 6, 8, 12, 24]
    )[0]
)
HISTO_DR_MB.append(
    divide_by_eachother_barlow(
        [HISTOS_LCOVERD0[4]], [HISTOS_LCOVERD0[0]], None, [1, 2, 4, 6, 8, 12, 24]
    )[0]
)
...
for mb, _ in enumerate(HISTO_DR_MB):
    ...
    ERRS_GR_DR_MB_TOT.append(
        Errors.make_root_asymm(HISTO_DR_MB[mb], tot_Lc_over_D0_DR, const_x_err=0.3)
    )
    ERRS_GR_DR_MB_FD.append(
        Errors.make_root_asymm(HISTO_DR_MB[mb], tot_Lc_over_D0_DR_FD, const_x_err=0.3)
    )
    ERRS_GR_DR_MB_WOFD.append(
        Errors.make_root_asymm(HISTO_DR_MB[mb], tot_Lc_over_D0_DR_WOFD, const_x_err=0.3)
    )
...
SAVE_PATH = make_standard_save_path(FILENAME, PATH_OUT)
save_histograms(
    [*HISTO_DR_MB, *ERRS_GR_DR_MB_TOT, *ERRS_GR_DR_MB_FD, *ERRS_GR_DR_MB_WOFD],
    SAVE_PATH,
)

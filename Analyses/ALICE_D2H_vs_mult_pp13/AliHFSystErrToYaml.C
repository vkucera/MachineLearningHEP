...
void AliHFSystErrToYaml(TString inputrootfile, TString outputyamlfile, Bool_t isLc = kFALSE)
{
  ...
  */
  ...
  // yield
  fOutYaml << "names: [\"yield\", \"cut\", \"track\", \"ptshape\", \"feeddown_mult\", \"trigger\", \"multiplicity_weights\", \"multiplicity_interval\", \"branching_ratio\", \"sigmav0\"]" << endl
           << endl;
  ...
  */
  ...
  for (int ipt = 0; ipt < nPtBins; ipt++) {
    if (!isLc)
      fOutYaml << "    - [0., 0., " << 0.0129 << ", " << 0.0129 << "]" << endl;
    else
      fOutYaml << "    - [0., 0., " << 0.053 << ", " << 0.053 << "]" << endl;

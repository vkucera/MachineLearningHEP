...
def results(
    ...
    **kwargs,
    ...
    if systematics and (len(histos_central) != len(systematics)):
        print(
            f"Number of systematics {len(systematics)} differs from number of "
    ...
    if len(histos_central) != len(legend_titles):
        print(
            f"Number of legend titles {len(legend_titles)} differs from number of "
            f"histograms {len(histos_central)}"
        )
        return
    ...
    if len(histos_central) != len(colors):
        print(
            f"Number of colors {len(colors)} differs from number of "
            f"histograms {len(histos_central)}"
        )
        return
    ...
    markerstyles = (
        [1] * len(histos_central) + [20] * len(histos_central)
        if systematics
        else [20] * len(histos_central)
    )
    draw_options = (
        ["E2"] * len(histos_central) + [""] * len(histos_central)
        if systematics
        else [""] * len(histos_central)
    )
    if systematics:
        colors = colors * 2
...
def extract_histo_or_error(
    case, ana_type, mult_bin, period_number, histo_name, filepath=None
):
...
LEGEND_TITLES = [
    "#kern[0.5]{0.0} #leq V0M_{percentile} #leq 100.0 (MB)",
...
for hn, yr in zip(HFPT_SPECTRUM_NAMES, HFPT_SPECTRUM_RANGES):
    ...
    DB_PATH_D0 = (
        "/home/bvolkel/HF/MachineLearningHEP/machine_learning_hep/data/"
    ...
    for mb in range(3):
        ...
        histo_lc.Scale(1.0 / SIGMAV0)
        histo_lc.Scale(1.0 / BRLC)
        histo_d0.Scale(1.0 / SIGMAV0)
        histo_d0.Scale(1.0 / BRD0)
    ...
    cross_section_std_D0 = (
        f"{cross_section_std_top_dir}/finalcrossD0ppMBvspt_ntrklmult0.root"
    )
    ...
    results(
        [histo_std_d0_new, histo_ml_d0_new],
        None,
        "",
        ["STD", "ML"],
        "#it{p}_{T} (GeV/#it{c})",

...
class MultiProcesser:  # pylint: disable=too-many-instance-attributes, too-many-statements
    ...
    def __init__(self, case, proc_class, datap, typean, run_param, mcordata):
        ...
        self.d_pklml_mergedallp = datap["multi"][self.mcordata][
        ...
        self.lptper_recoml = [
            [os.path.join(direc, self.lpt_recosk[ipt]) for direc in self.dlper_pklml]
            for ipt in range(self.p_nptbins)
        ]
        self.lper_evtml = [
            os.path.join(direc, self.n_evt) for direc in self.dlper_pklml
        ]
        self.lper_evtorigml = [
            os.path.join(direc, self.n_evtorig) for direc in self.dlper_pklml
        ]
        self.lptper_genml = [
            [os.path.join(direc, self.lpt_gensk[ipt]) for direc in self.dlper_pklml]
            for ipt in range(self.p_nptbins)
        ]
        ...
        self.f_evtorigml_mergedallp = os.path.join(
            self.d_pklml_mergedallp, self.n_evtorig
        )
        ...
        self.dlper_reco_modapp = datap["mlapplication"][self.mcordata][
            "pkl_skimmed_dec"
        ]
        self.dlper_reco_modappmerged = datap["mlapplication"][self.mcordata][
        ...
        self.f_evtorig_mergedallp = os.path.join(
            self.d_pklevt_mergedallp, self.n_evtorig
        )

        self.lper_runlistrigger = datap["analysis"][self.typean][self.mcordata][
            "runselection"
        ]

        self.lper_mcreweights = None
        if self.mcordata == "mc":
            self.lper_mcreweights = [
                os.path.join(direc, self.n_mcreweights)
    ...
    def multi_histomass(self):
        ...
        tmp_merged = (
            f"/data/tmp/hadd/{self.case}_{self.typean}/mass/{get_timestamp_string()}/"
        )
    ...
    def multi_efficiency(self):
        ...
        tmp_merged = f"/data/tmp/hadd/{self.case}_{self.typean}/efficiency/{get_timestamp_string()}/"
    ...
    def multi_response(self):
        ...
        if resp_exists:
            tmp_merged = f"/data/tmp/hadd/{self.case}_{self.typean}/response/{get_timestamp_string()}/"

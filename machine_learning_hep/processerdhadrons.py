...
from machine_learning_hep.utilities import openfile, seldf_singlevar, selectdfrunlist
...
class ProcesserDhadrons(Processer):  # pylint: disable=too-many-instance-attributes
    ...
    def process_histomass_single(self, index):
        ...
        hsel, hnovtxmult, hvtxoutmult = gethistonormforselevt_varsel(
            dfevtorig, dfevtevtsel, labeltrigger, self.s_var_evt_sel
        )
        hsel.Write()
        hnovtxmult.Write()
        hvtxoutmult.Write()
        ...
        for ipt in range(self.p_nptfinbins):
            ...
            if self.mcordata == "mc":
                df[self.v_ismcrefl] = np.array(
                    tag_bit_df(df, self.v_bitvar, self.b_mcrefl), dtype=int
                )
    ...
    def process_efficiency_single(self, index):
        ...
        h_gen_fd = TH1F(
            "h_gen_fd", "FD Generated in acceptance |y|<0.5", n_bins, analysis_bin_lims
        )
        h_presel_fd = TH1F(
            "h_presel_fd",

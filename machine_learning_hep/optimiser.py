...
from machine_learning_hep.mlperformance import (
    cross_validation_mse,
    plot_cross_validation_mse,
...
from machine_learning_hep.optimisation.grid_search import (
    do_gridsearch,
    perform_plot_gridsearch,
)
from machine_learning_hep.optimization import (
    calc_bkg,
    calc_eff,
    calc_sigeff_steps,
    calc_signif,
...
class Optimiser:
    ...
    def __init__(
        ...
        if self.is_fonll_from_root and "fonll_particle" not in data_param["ml"]["opt"]:
            self.logger.fatal(
                "Attempt to read FONLL from ROOT file but field "
        ...
        self.f_reco_applieddata = self.f_reco_applieddata.replace(
            ".pkl", "%s.pkl" % self.s_suffix
        )
        self.f_reco_appliedmc = self.f_reco_appliedmc.replace(
            ".pkl", "%s.pkl" % self.s_suffix
        )
    ...
    def create_suffix(self):
        string_selection = createstringselection(
    ...
    def preparesample(self):
        ...
        self.df_mc = seldf_singlevar(
            self.df_mc, self.v_bin, self.p_binmin, self.p_binmax
        )
        self.df_mcgen = seldf_singlevar(
            self.df_mcgen, self.v_bin, self.p_binmin, self.p_binmax
        )
        self.df_data = seldf_singlevar(
            self.df_data, self.v_bin, self.p_binmin, self.p_binmax
        )

        self.df_sig, self.df_bkg = arraydf[self.p_tagsig], arraydf[self.p_tagbkg]
        self.df_sig = seldf_singlevar(
            self.df_sig, self.v_bin, self.p_binmin, self.p_binmax
        )
        self.df_bkg = seldf_singlevar(
            self.df_bkg, self.v_bin, self.p_binmin, self.p_binmax
        )
        ...
        self.logger.info(
            "Total number of candidates: train %d and test %d",
            len(self.df_mltrain),
            len(self.df_mltest),
        )
        self.logger.info(
            "Number of signal candidates: train %d and test %d",
            len(self.df_sigtrain),
            len(self.df_sigtest),
        )
        self.logger.info(
            "Number of bkg candidates: %d and test %d",
            len(self.df_bkgtrain),
            len(self.df_bkgtest),
        )
    ...
    def do_corr(self):
        imageIO_vardist_all = vardistplot(
            self.df_sigtrain,
            self.df_bkgtrain,
            self.v_all,
            self.dirmlplot,
        ...
        imageIO_vardist_train = vardistplot(
            self.df_sigtrain,
            self.df_bkgtrain,
            self.v_train,
            self.dirmlplot,
        ...
        imageIO_scatterplot = scatterplot(
            self.df_sigtrain,
            self.df_bkgtrain,
            self.v_corrx,
            self.v_corry,
        ...
        imageIO_corr_sig_all = correlationmatrix(
            self.df_sigtrain,
            self.v_all,
            "Signal",
            self.dirmlplot,
        ...
        imageIO_corr_bkg_all = correlationmatrix(
            self.df_bkgtrain,
            self.v_all,
            "Background",
            self.dirmlplot,
        ...
        imageIO_corr_sig_train = correlationmatrix(
            self.df_sigtrain,
            self.v_train,
            "Signal",
            self.dirmlplot,
        ...
        imageIO_corr_bkg_train = correlationmatrix(
            self.df_bkgtrain,
            self.v_train,
            "Background",
            self.dirmlplot,
        ...
        return (
            imageIO_vardist_all,
            imageIO_vardist_train,
            imageIO_scatterplot,
            imageIO_corr_sig_all,
            imageIO_corr_bkg_all,
            imageIO_corr_sig_train,
    ...
    def loadmodels(self):
        ...
        classifiers_keras, names_keras, _, _ = getclf_keras(
    ...
    def do_train(self):
        ...
        t0 = time.time()
        self.p_trainedmod = fit(
            self.p_classname, self.p_class, self.df_xtrain, self.df_ytrain
        )
    ...
    def do_apply(self):
        ...
        df_mc = apply(
            self.p_mltype, self.p_classname, self.p_trainedmod, self.df_mc, self.v_train
        )
    ...
    def do_crossval(self):
        ...
        plot_cross_validation_mse(
            self.p_classname, df_scores, self.s_suffix, self.dirmlplot
        )
    ...
    def do_importance(self):
        importanceplotall(
            self.v_train, self.p_classname, self.p_class, self.s_suffix, self.dirmlplot
        )
    ...
    def do_bayesian_opt(self):
        ...
        out_dirs = [
            os.path.join(self.dirmlplot, "bayesian_opt", name, f"{name}{self.s_suffix}")
            for name in clfs_names_all
        ]
        if checkdirlist(out_dirs):
            # Only draw results if any can be found
            self.logger.warning(
                "Not overwriting anything, just plotting if possible "
    ...
    def do_grid(self):
        ...
        clfs_xgboost, names_xgboost, grid_params_xgboost, _ = getclf_xgboost(
            self.db_model
        )
        clfs_keras, names_keras, grid_params_keras, _ = getclf_keras(
            self.db_model, len(self.df_xtrain.columns)
        )
        clfs_grid_params_all = (
            grid_params_scikit + grid_params_xgboost + grid_params_keras
        )
        ...
        clfs_names_all = [
            name for name, gps in zip(clfs_names_all, clfs_grid_params_all) if gps
        ]
        clfs_grid_params_all = [gps for gps in clfs_grid_params_all if gps]

        out_dirs = [
            os.path.join(self.dirmlplot, "grid_search", name, f"{name}{self.s_suffix}")
            for name in clfs_names_all
        ]
        if checkdirlist(out_dirs):
            # Only draw results if any can be found
    ...
    def do_boundary(self):
        ...
        trainedmodels_2var = fit(
            names_2var, classifiers_2var, x_test_boundary, self.df_ytest
        )
        decisionboundaries(
            names_2var,
            trainedmodels_2var,
    ...
    def do_significance(self):
        ...
        if self.is_fonll_from_root:
            ...
            df_fonll_Lc = df_fonll.Get(
                self.p_fonllparticle + "pred_" + self.p_fonllband
            )
            prod_cross = (
                df_fonll_Lc.Integral(ptmin * 20, ptmax * 20)
                * self.p_fragf
                * 1e-12
                / delta_pt
            )
            ...
            df_fonll_in_pt = df_fonll.query("(pt >= @ptmin) and (pt < @ptmax)")[
                self.p_fonllband
            ]
            ...
            plt.subplot(111)
            plt.plot(
                df_fonll["pt"], df_fonll[self.p_fonllband] * self.p_fragf, linewidth=4.0
            )
        ...
        hmass = TH1F(
            "hmass", "", self.p_num_bins, self.p_mass_fit_lim[0], self.p_mass_fit_lim[1]
        )
        ...
        gaus_fit = TF1(
            "gaus_fit", "gaus", self.p_mass_fit_lim[0], self.p_mass_fit_lim[1]
        )
        ...
        for name in self.p_classname:
            ...
            bkg_err_array = [
                bkg_err / (self.p_bkgfracopt * self.p_nevtml)
            ...
            with open(
                f"{self.dirmlplot}/Significance_{self.s_suffix}.pickle", "wb"
            ) as out:

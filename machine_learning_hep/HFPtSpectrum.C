...
enum datayear { k2010,
                ...
                k2017,
                k2018 };
enum BFDSubtrMethod { knone,
                      kfc,
                      kNb };
enum RaavsEP { kPhiIntegrated,
               kInPlane,
               kOutOfPlane };
enum rapidity { kdefault,
                k08to04,
                k07to04,
                k04to01,
                k01to01,
                ...
                k04to08,
                k01to05 };
enum particularity { kTopological,
                     kLowPt,
                     kPP7TeVPass4,
                     kBDT };
                  ...
                  const char* nevhistoname = "hNEvents")
{
  ...
  if (rapiditySlice != kdefault) {
    ...
    switch (rapiditySlice) {
      case k08to04:
        scaleFONLL = (0.093 + 0.280) / 1.0;
        break;
      case k07to04:
        scaleFONLL = 0.280 / 1.0;
        break;
      case k04to01:
        scaleFONLL = 0.284 / 1.0;
        break;
      case k01to01:
        scaleFONLL = 0.191 / 1.0;
        break;
      case k01to04:
        scaleFONLL = 0.288 / 1.0;
        break;
      case k04to07:
        scaleFONLL = 0.288 / 1.0;
        break;
      case k04to08:
        scaleFONLL = (0.288 + 0.096) / 1.0;
        break;
      case k01to05:
        scaleFONLL = (0.4) / 1.0;
        break;
    ...
    switch (rapiditySlice) {
      case k08to04:
        scaleFONLL = (0.089 + 0.274) / 1.0;
        break;
      case k07to04:
        scaleFONLL = 0.274 / 1.0;
        break;
      case k04to01:
        scaleFONLL = 0.283 / 1.0;
        break;
      case k01to01:
        scaleFONLL = 0.192 / 1.0;
        break;
      case k01to04:
        scaleFONLL = 0.290 / 1.0;
        break;
      case k04to07:
        scaleFONLL = 0.291 / 1.0;
        break;
      case k04to08:
        scaleFONLL = (0.291 + 0.097) / 1.0;
        break;
      case k01to05:
        scaleFONLL = (0.4) / 1.0;
        break;
  ...
  if (isRaavsEP > 0.) {
    ...
    if (isRaavsEP == kInPlane)
      hEPresolCorr = (TH1D*)EPf->Get("hCorrEPresol_InPlane");
    else if (isRaavsEP == kOutOfPlane)
      hEPresolCorr = (TH1D*)EPf->Get("hCorrEPresol_OutOfPlane");
  ...
  if (option == 1) {
    ...
    if (asym)
      spectra->SetMCptDistributionsBounds(hDirectMCptMax, hDirectMCptMin, hFeedDownMCptMax, hFeedDownMCptMin);
  } else if (option == 2) {
    spectra->SetFeedDownMCptSpectra(hFeedDownMCpt);
    if (asym)
      spectra->SetFeedDownMCptDistributionsBounds(hFeedDownMCptMax, hFeedDownMCptMin);
  }
  ...
  if (isRaavsEP > 0.)
    spectra->SetIsEventPlaneAnalysis(kTRUE);
  ...
  if (year == k2010)
    systematics->SetRunNumber(10);
  else if (year == k2011)
    systematics->SetRunNumber(11);
  else if (year == k2012)
    systematics->SetRunNumber(12);
  else if (year == k2013)
    systematics->SetRunNumber(13);
  else if (year == k2015)
    systematics->SetRunNumber(15);
  else if (year == k2016)
    systematics->SetRunNumber(16);
  else if (year == k2017)
  ...
  if (cc == kpp276) {
    ...
    if (ccestimator == kV0A) {
      if (cc == kpPb020)
        systematics->SetCentrality("020V0A");
      else if (cc == kpPb2040)
        systematics->SetCentrality("2040V0A");
      else if (cc == kpPb4060)
        systematics->SetCentrality("4060V0A");
      else if (cc == kpPb60100)
        systematics->SetCentrality("60100V0A");
    } else if (ccestimator == kZNA) {
      if (cc == kpPb010) {
        systematics->SetCentrality("010ZNA");
      } else if (cc == kpPb020)
        systematics->SetCentrality("020ZNA");
      else if (cc == kpPb1020)
        systematics->SetCentrality("1020ZNA");
      else if (cc == kpPb2040)
        systematics->SetCentrality("2040ZNA");
      else if (cc == kpPb4060)
        systematics->SetCentrality("4060ZNA");
      else if (cc == kpPb60100)
        systematics->SetCentrality("60100ZNA");
    } else if (ccestimator == kCL1) {
      if (cc == kpPb020)
        systematics->SetCentrality("020CL1");
      else if (cc == kpPb2040)
        systematics->SetCentrality("2040CL1");
      else if (cc == kpPb4060)
        systematics->SetCentrality("4060CL1");
      else if (cc == kpPb60100)
        systematics->SetCentrality("60100CL1");
    } else {
      if (!(cc == kpPb0100)) {
  ...
  //
    ...
    if (Energy == k276) {
      if (cc == k07half)
        systematics->SetCentrality("07half");
      else if (cc == k010)
        systematics->SetCentrality("010");
      else if (cc == k1020)
        systematics->SetCentrality("1020");
      else if (cc == k020)
        ...
        if (isRaavsEP == kPhiIntegrated)
          systematics->SetCentrality("4080");
        else if (isRaavsEP == kInPlane)
          systematics->SetCentrality("3050InPlane");
        else if (isRaavsEP == kOutOfPlane)
          systematics->SetCentrality("3050OutOfPlane");
      } else if (cc == k4060 || cc == k4050 || cc == k5060)
        systematics->SetCentrality("4060");
      else if (cc == k6080)
  ...
  // the efficiencies
  if (!hDirectEffpt)
    hDirectEffpt = (TH1D*)spectra->GetDirectAccEffCorrection();
  if (!hFeedDownEffpt)
    hFeedDownEffpt = (TH1D*)spectra->GetFeedDownAccEffCorrection();
  ...
  gSigmaCorr->Write();
  if (asym) {
    if (gYieldCorrExtreme)
      gYieldCorrExtreme->Write();
    if (gSigmaCorrExtreme)
      gSigmaCorrExtreme->Write();
    if (gYieldCorrConservative)
      gYieldCorrConservative->Write();
    if (gSigmaCorrConservative)
      gSigmaCorrConservative->Write();
    if (asym && gFcConservative)
                   ...
                   const char* nevhistoname = "hNEvents")
{
  ...
  for (Int_t ibin = 1; ibin <= fnPtBins; ibin++) {
    ...
    if (hRECpt->GetBinContent(ibin) > 0. && hRECpt->GetBinContent(ibin) != 0. && hFeedDownMCpt->GetBinContent(ibin) > 0. && hFeedDownEffpt->GetBinContent(ibin) > 0.)
      value = hRECpt->GetBinContent(ibin) * correction;
    value /= hRECpt->GetBinWidth(ibin);
    if (value < 0.)
      value = 0.;

    //  Statistical uncertainty:   delta_physics = sqrt ( (delta_reco)^2 )  / bin-width
    if (value != 0. && hRECpt->GetBinError(ibin) && hRECpt->GetBinError(ibin) != 0.)
      errvalue = hRECpt->GetBinError(ibin);
  ...
  //
  if (isParticlePlusAntiParticleYield)
  ...
  for (Int_t ibin = 1; ibin <= fnPtBins; ibin++) {
    ...
    errvalueMax = 0.;
    errvalueMin = 0.;
    errvalueExtremeMax = 0.;
    errvalueExtremeMin = 0.;
    errvalueConservativeMax = 0.;
    errvalueConservativeMin = 0.;
    ...
    if (value > 0.)
      hStatUncEffcSigma->SetBinError(ibin, ((errvalueStatUncEffc / value) * 100.));
    hStatUncEffbSigma->SetBinContent(ibin, 0.);
    hStatUncEffbSigma->SetBinError(ibin, 0.);
  }

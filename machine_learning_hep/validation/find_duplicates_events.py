...
for dm in DATA_MC:
    ...
    for period, dir_applied in zip(
        DATABASE["multi"][dm]["period"], DATABASE["multi"][dm]["pkl"]
    ):
        ...
        for child in children:
            ...
            duplicates_ratio = [
                d[1] / d[0] * 100 if d[0] > 0 else 0.0 for d in duplicates
            ]
            ...
            if EXTRACT_DUPL_INFO:
                ...
                for d in duplicates:
                    ...
                    for _, row in d[2].iterrows():
                        duplicates_cols_this_df.append(
                            [float(row[col_name]) for col_name in UNIQUE_COLS]
                        )
            ...
            DUPLICATES_SUMMARY[dm][period][child] = [
                ...
                for df, dr, hd, dc in zip(
                    files_child, duplicates_ratio, has_duplicates, duplicates_cols
                )
            ]

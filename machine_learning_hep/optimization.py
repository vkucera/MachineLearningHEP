...
def calc_bkg(
    ...
    if save_fit:
        ...
        pt_max = pt_lims[1]
        out_file = TFile(
            f"{out_dir}/bkg_fits_{name}_pt{pt_min:.1f}_{pt_max:.1f}.root", "recreate"
        )
        out_file.cd()
    ...
    for thr in x_axis:
        ...
        if len(sel_mass_array) > 5:
            ...
            if int(fit) == 0:
                ...
                bkg_err = (
                    fit_func.IntegralError(sig_region[0], sig_region[1]) / bin_width
                )
                del fit_func
        elif save_fit:
            hmass.Write()
...
def calc_signif(sig_array, sig_err_array, bkg_array, bkg_err_array):
    ...
    for sig, bkg, sig_err, bkg_err in zip(
        sig_array, bkg_array, sig_err_array, bkg_err_array
    ):
        signif = 0.0
        signif_err = 0.0
        ...
        if sig > 0 and (sig + bkg) > 0:
            ...
            signif_err = signif * np.sqrt(
                (sig_err ** 2 + bkg_err ** 2) / (4 * (sig + bkg) ** 2)

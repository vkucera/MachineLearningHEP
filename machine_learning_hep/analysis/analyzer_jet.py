...
from machine_learning_hep.processerdhadrons_jet import (
    adjust_nsd,
    adjust_rg,
    adjust_z,
    adjust_zg,
...
class AnalyzerJet(Analyzer):
    ...
    def __init__(self, datap, case, typean, period):
        ...
        self.branching_ratio = datap["analysis"][self.typean].get(
        ...
        self.var2binarray_reco = array(
            "d", self.var2ranges_reco
        )  # array of bin edges to use in histogram constructors
        self.var2ranges_gen = self.lvar2_binmin_gen.copy()
        self.var2ranges_gen.append(self.lvar2_binmax_gen[-1])
        self.var2binarray_gen = array(
            "d", self.var2ranges_gen
        )  # array of bin edges to use in histogram constructors

        # observable (z, shape,...)
        self.v_varshape_binning = datap["analysis"][self.typean][
            "var_binningshape"
        ]  # name (reco)
        self.v_varshape_binning_gen = datap["analysis"][self.typean][
            "var_binningshape_gen"
        ]  # name (gen)
        self.lvarshape_binmin_reco = datap["analysis"][self.typean].get(
            "sel_binminshape_reco", None
        )
        self.lvarshape_binmax_reco = datap["analysis"][self.typean].get(
            "sel_binmaxshape_reco", None
        )
        self.p_nbinshape_reco = len(self.lvarshape_binmin_reco)  # number of reco bins
        self.lvarshape_binmin_gen = datap["analysis"][self.typean].get(
            "sel_binminshape_gen", None
        )
        self.lvarshape_binmax_gen = datap["analysis"][self.typean].get(
            "sel_binmaxshape_gen", None
        )
        ...
        self.varshapebinarray_reco = array(
            "d", self.varshaperanges_reco
        )  # array of bin edges to use in histogram constructors
        self.varshaperanges_gen = self.lvarshape_binmin_gen.copy()
        self.varshaperanges_gen.append(self.lvarshape_binmax_gen[-1])
        self.varshapebinarray_gen = array(
            "d", self.varshaperanges_gen
        )  # array of bin edges to use in histogram constructors
        ...
        self.sideband_sigma_1_left = datap["analysis"][self.typean].get(
            "sideband_sigma_1_left", None
        )
        self.sideband_sigma_1_right = datap["analysis"][self.typean].get(
            "sideband_sigma_1_right", None
        )
        self.sideband_sigma_2_left = datap["analysis"][self.typean].get(
            "sideband_sigma_2_left", None
        )
        self.sideband_sigma_2_right = datap["analysis"][self.typean].get(
            "sideband_sigma_2_right", None
        )
        self.sigma_scale = datap["analysis"][self.typean].get("sigma_scale", None)
        self.sidebandleftonly = datap["analysis"][self.typean].get(
            "sidebandleftonly", None
        )

        # feed-down
        self.powheg_path_nonprompt = datap["analysis"][self.typean].get(
            "powheg_path_nonprompt", None
        )
        # systematics variations

        # models to compare with
        # POWHEG + PYTHIA 6
        self.powheg_path_prompt = datap["analysis"][self.typean].get(
            "powheg_path_prompt", None
        )
        self.powheg_prompt_variations = datap["analysis"][self.typean].get(
            "powheg_prompt_variations", None
        )
        self.powheg_prompt_variations_path = datap["analysis"][self.typean].get(
            "powheg_prompt_variations_path", None
        )
        # PYTHIA 8
        self.pythia8_prompt_variations_path = datap["analysis"][self.typean].get(
            "pythia8_prompt_variations_path", None
        )
        self.pythia8_prompt_variations = datap["analysis"][self.typean].get(
            "pythia8_prompt_variations", None
        )
        self.pythia8_prompt_variations_legend = datap["analysis"][self.typean].get(
            "pythia8_prompt_variations_legend", None
        )

        # unfolding
        self.niter_unfolding = datap["analysis"][self.typean].get(
            "niterunfolding", None
        )
        self.choice_iter_unfolding = datap["analysis"][self.typean].get(
            "niterunfoldingchosen", None
        )
        ...
        self.systematic_catnames = [
            catname for catname, val in db_sys.items() if val["activate"]
        ]
        ...
        if "data_proc" in datap["analysis"][typean]:
            self.d_resultsallpdata_proc = (
                datap["analysis"][typean]["data_proc"]["results"][period]
                if period is not None
                else datap["analysis"][typean]["data_proc"]["resultsallp"]
            )
        if "mc_proc" in datap["analysis"][typean]:
            self.d_resultsallpmc_proc = (
                datap["analysis"][typean]["mc_proc"]["results"][period]
                if period is not None
                else datap["analysis"][typean]["mc_proc"]["resultsallp"]
            )
        ...
        self.file_sideband = os.path.join(
            self.d_resultsallpdata, "sideband_subtracted.root"
        )
        self.file_feeddown = os.path.join(self.d_resultsallpdata, "feeddown.root")
        self.file_unfold = os.path.join(
            self.d_resultsallpdata, "unfolding_results.root"
        )
        self.file_unfold_closure = os.path.join(
            self.d_resultsallpdata, "unfolding_closure.root"
        )
        ...
        self.title_full_ratio = ";%s;data/MC: ratio of %s" % (
            self.title_x,
            self.title_y,
        )
        # text
        ...
        self.text_jets = (
            "%s-tagged charged jets, anti-#it{k}_{T}, #it{R} = 0.4"
            % self.p_latexnhadron
        )
        self.text_ptjet = (
    ...
    def fit(self):
        ...
        for ipt in range(self.p_nptfinbins):
            ...
            for ibin2 in range(self.p_nbin2_reco):
                ...
                if not histomassmc:
                    self.logger.fatal(
                        make_message_notfound("hmass_sig" + suffix, self.n_filemass_mc)
                    )
                histomassmc_reb = AliVertexingHFUtils.RebinHisto(
                    histomassmc, self.p_rebin[ipt], -1
                )
                histomassmc_reb_f = TH1F()
                histomassmc_reb.Copy(histomassmc_reb_f)
                fittermc = AliHFInvMassFitter(
                    histomassmc_reb_f,
                    self.p_massmin[ipt],
                    self.p_massmax[ipt],
                    self.p_bkgfunc[ipt],
                ...
                if not histomass:
                    self.logger.fatal(
                        make_message_notfound("hmass" + suffix, self.n_filemass)
                    )
                histomass_reb = AliVertexingHFUtils.RebinHisto(
                    histomass, self.p_rebin[ipt], -1
                )
                histomass_reb_f = TH1F()
                histomass_reb.Copy(histomass_reb_f)
                fitter = AliHFInvMassFitter(
                    histomass_reb_f,
                    self.p_massmin[ipt],
                    self.p_massmax[ipt],
                    self.p_bkgfunc[ipt],
                ...
                y_margin_up = 0.15
                y_margin_down = 0.05
                histomass_reb.GetYaxis().SetRangeUser(
                    *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
                )
                ...
                latex2 = TLatex(
                    ...
                    % (
                        self.lpt_finbinmin[ipt],
                        self.p_latexnhadron,
                        min(self.lpt_finbinmax[ipt], self.lvar2_binmax_reco[ibin2]),
                    ),
                )
                draw_latex(latex2)
                c_fitted_result.SaveAs(
                    "%s/fit_%s.eps" % (self.d_resultsallpdata, suffix_plot)
                )
        myfilemc.Close()
        myfile.Close()
        fileout.Close()
    ...
    def efficiency(self):  # pylint: disable=too-many-branches, too-many-locals
        ...
        string_shaperange = "%g #leq %s < %g" % (
            self.lvarshape_binmin_reco[0],
            self.v_varshape_latex,
            self.lvarshape_binmax_reco[-1],
        )
        ...
        if not hzvsjetpt_reco_nocuts:
            self.logger.fatal(
                make_message_notfound("hzvsjetpt_reco_nocuts", self.n_fileresp)
            )
        # rec. level and gen. level cuts applied
        hzvsjetpt_reco_eff = lfileresp.Get("hzvsjetpt_reco_cuts")
        if not hzvsjetpt_reco_eff:
            self.logger.fatal(
                make_message_notfound("hzvsjetpt_reco_cuts", self.n_fileresp)
            )
        ...
        if not hzvsjetpt_gen_nocuts:
            self.logger.fatal(
                make_message_notfound("hzvsjetpt_gen_nocuts", self.n_fileresp)
            )
        # rec. level and gen. level cuts applied
        hzvsjetpt_gen_eff = lfileresp.Get("hzvsjetpt_gen_cuts")
        if not hzvsjetpt_gen_eff:
            self.logger.fatal(
                make_message_notfound("hzvsjetpt_gen_cuts", self.n_fileresp)
            )
        ...
        h2_ptcand_ptjet_gen_overflow_folded = buildhisto(
            "h2_ptcand_ptjet_gen_overflow_folded",
            "h2_ptcand_ptjet_gen_overflow_folded",
            self.var1binarray,
            self.var2binarray_reco,
        )
        h2_ptcand_ptjet_gen_folded = buildhisto(
            "h2_ptcand_ptjet_gen_folded",
            "h2_ptcand_ptjet_gen_folded",
            self.var1binarray,
            self.var2binarray_reco,
        )
        ...
        output_template = buildhisto(
            "h2_z_jetpt",
            "h2_z_jetpt",
            self.varshapebinarray_reco,
            self.var2binarray_reco,
        )
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            h1_ptcand_rec_overflow = h2_ptcand_ptjet_rec_overflow.ProjectionX(
                "h1_ptcand_rec_overflow_%d" % ibin2, ibin2 + 1, ibin2 + 1
            )
            list_ptcand_rec_new_overflow.append(h1_ptcand_rec_overflow)
            h1_ptcand_gen_overflow = h3_z_ptjet_ptcand_gen.Project3D(
                "h1_ptcand_gen_%d_overflow_ze" % ibin2
            )
            list_ptcand_gen_new_overflow.append(h1_ptcand_gen_overflow)
            # rec level efficiency
            h1_ptcand_eff_overflow = h1_ptcand_rec_overflow.Clone(
                "h1_ptcand_eff_overflow_%d" % ibin2
            )
            ...
            h1_ptcand_genmatched_overflow = (
                h2_ptcand_ptjet_genmatched_overflow.ProjectionX(
                    "h1_ptcand_genmatched_overflow_%d" % ibin2, ibin2 + 1, ibin2 + 1
                )
            )
            list_ptcand_genmatched_new_overflow.append(h1_ptcand_genmatched_overflow)
            h1_ptcand_effgen_overflow = h1_ptcand_genmatched_overflow.Clone(
                "h1_ptcand_effgen_overflow_%d" % ibin2
            )
            ...
            h1_ptcand_rec = h2_ptcand_ptjet_rec.ProjectionX(
                "h1_ptcand_rec_%d" % ibin2, ibin2 + 1, ibin2 + 1
            )
            list_ptcand_rec_new.append(h1_ptcand_rec)
            h3_z_ptjet_ptcand_gen.GetXaxis().SetRange(
                1, h3_z_ptjet_ptcand_gen.GetXaxis().GetNbins()
            )
            h1_ptcand_gen = h3_z_ptjet_ptcand_gen.Project3D(
                "h1_ptcand_gen_%d_ze" % ibin2
            )
            ...
            # gen level efficiency
            h1_ptcand_genmatched = h2_ptcand_ptjet_genmatched.ProjectionX(
                "h1_ptcand_genmatched_%d" % ibin2, ibin2 + 1, ibin2 + 1
            )
        ...
        for ipt in range(self.p_nptfinbins):
            ...
            # overflow projection
            h1_ptjet_rec_overflow = h2_ptcand_ptjet_rec_overflow.ProjectionY(
                "h1_ptjet_rec_overflow_%d" % ipt, ipt + 1, ipt + 1, "e"
            )
            # restricted projection
            h1_ptjet_rec = h2_ptcand_ptjet_rec.ProjectionY(
                "h1_ptjet_rec_%d" % ipt, ipt + 1, ipt + 1, "e"
            )

            # get the shape_gen vs pt_gen_jet projection for the given pt_cand bin
            h3_z_ptjet_ptcand_gen.GetZaxis().SetRange(ipt + 1, ipt + 1)
            # overflow projection
            h2_z_ptjet_gen_overflow_orig = h3_z_ptjet_ptcand_gen.Project3D(
                "h2_shape_jetpt_ptcand_%d_gen_overflow_yxe" % ipt
            )
            h2_z_ptjet_gen_overflow = h2_z_ptjet_gen_overflow_orig.Clone(
                h2_z_ptjet_gen_overflow_orig.GetName() + "_clone"
            )
            # restricted projection
            h3_z_ptjet_ptcand_gen.GetXaxis().SetRange(
                1, h3_z_ptjet_ptcand_gen.GetXaxis().GetNbins()
            )  # restrict shape range
            h2_z_ptjet_gen_orig = h3_z_ptjet_ptcand_gen.Project3D(
                "h2_shape_jetpt_ptcand_%d_gen_yxe" % ipt
            )
            h2_z_ptjet_gen = h2_z_ptjet_gen_orig.Clone(
                h2_z_ptjet_gen_orig.GetName() + "_clone"
            )
            ...
            h2_z_ptjet_gen_overflow_folded = folding(
                h2_z_ptjet_gen_overflow, response_matrix, output_template
            )
            h2_z_ptjet_gen_folded = folding(
                h2_z_ptjet_gen, response_matrix, output_template
            )
            ...
            # overflow projection
            h1_ptjet_gen_overflow_orig = h2_z_ptjet_gen_overflow_orig.ProjectionY(
                "h1_ptjet_gen_overflow_orig_%d" % ipt, 0, self.p_nbinshape_gen + 1, "e"
            )
            h1_ptjet_gen_overflow_folded = h2_z_ptjet_gen_overflow_folded.ProjectionY(
                "h1_ptjet_gen_overflow_folded_%d" % ipt,
                0,
                self.p_nbinshape_reco + 1,
                "e",
            )
            # restricted projection
            h1_ptjet_gen_orig = h2_z_ptjet_gen_orig.ProjectionY(
                "h1_ptjet_gen_orig_%d" % ipt, 1, self.p_nbinshape_gen, "e"
            )
            h1_ptjet_gen_folded = h2_z_ptjet_gen_folded.ProjectionY(
                "h1_ptjet_gen_folded_%d" % ipt, 1, self.p_nbinshape_reco, "e"
            )
            ...
            list_obj = [
                h1_ptjet_rec_overflow,
                h1_ptjet_rec,
                h1_ptjet_gen_overflow_orig,
                h1_ptjet_gen_overflow_folded,
                h1_ptjet_gen_orig,
                h1_ptjet_gen_folded,
                latex,
                latex_shaperange,
            ...
            make_plot(
                "efficiency_pr_ptcand_%d" % ipt,
                path=self.d_resultsallpdata,
                list_obj=list_obj,
                labels_obj=labels_obj,
                title="new efficiency calculation;%s (GeV/#it{c});count"
                % self.p_latexbin2var,
                leg_pos=[0.55, 0.72, 0.8, 0.86],
                logscale="y",
                margins_y=[0.05, 0.25],
                colours=[get_colour(i) for i in (1, 3, 1, 2, 3, 4)],
                markers=[get_marker(i) for i in (0, 0, 1, 2, 3, 4)],
            )

            # fill the (pt_cand, pt_jet) bins of gen_folded
            for ibin2 in range(self.p_nbin2_reco):
                # overflow folded
                h2_ptcand_ptjet_gen_overflow_folded.SetBinContent(
                    ipt + 1,
                    ibin2 + 1,
                    h1_ptjet_gen_overflow_folded.GetBinContent(ibin2 + 1),
                )
                h2_ptcand_ptjet_gen_overflow_folded.SetBinError(
                    ipt + 1,
                    ibin2 + 1,
                    h1_ptjet_gen_overflow_folded.GetBinError(ibin2 + 1),
                )
                # restricted folded
                h2_ptcand_ptjet_gen_folded.SetBinContent(
                    ipt + 1, ibin2 + 1, h1_ptjet_gen_folded.GetBinContent(ibin2 + 1)
                )
                h2_ptcand_ptjet_gen_folded.SetBinError(
                    ipt + 1, ibin2 + 1, h1_ptjet_gen_folded.GetBinError(ibin2 + 1)
                )

        # calculate the efficieny (pt_cand, pt_jet) as eff = rec_matched/gen_folded
        # overflow folded
        h2_ptcand_ptjet_eff_overflow_folded = h2_ptcand_ptjet_rec_overflow.Clone(
            "h2_ptcand_ptjet_eff_overflow_folded"
        )
        h2_ptcand_ptjet_eff_overflow_folded.Divide(h2_ptcand_ptjet_gen_overflow_folded)
        # restricted folded
        h2_ptcand_ptjet_eff_folded = h2_ptcand_ptjet_rec.Clone(
            "h2_ptcand_ptjet_eff_folded"
        )
        ...
        for ibin2 in range(self.p_nbin2_reco):
            # overflow folded
            h1_ptcand_gen_overflow_folded = (
                h2_ptcand_ptjet_gen_overflow_folded.ProjectionX(
                    "h1_ptcand_gen_overflow_folded_%d" % ibin2,
                    ibin2 + 1,
                    ibin2 + 1,
                    "e",
                )
            ...
            h1_ptcand_eff_overflow_folded = (
                h2_ptcand_ptjet_eff_overflow_folded.ProjectionX(
                    "h1_ptcand_eff_overflow_folded_%d" % ibin2,
                    ibin2 + 1,
                    ibin2 + 1,
                    "e",
                )
            ...
            # restricted folded
            h1_ptcand_gen_folded = h2_ptcand_ptjet_gen_folded.ProjectionX(
                "h1_ptcand_gen_folded_%d" % ibin2, ibin2 + 1, ibin2 + 1, "e"
            )
            list_ptcand_gen_new_folded.append(h1_ptcand_gen_folded)
            h1_ptcand_eff_folded = h2_ptcand_ptjet_eff_folded.ProjectionX(
                "h1_ptcand_eff_folded_%d" % ibin2, ibin2 + 1, ibin2 + 1, "e"
            )
        ...
        setup_canvas(cEff)
        legeff = TLegend(0.13, 0.65, 0.5, 0.88)
        setup_legend(legeff)
        list_his = []
        list_ptcand_gen_old = []
        list_ptcand_rec_old = []
        ...
        latex_shaperange = TLatex(0.13, 0.8, string_shaperange)
        for ibin2 in range(self.p_nbin2_reco):
            string_ptjet = "%g #leq %s < %g GeV/#it{c}" % (
                self.lvar2_binmin_reco[ibin2],
                self.p_latexbin2var,
                self.lvar2_binmax_reco[ibin2],
            )
            latex = TLatex(0.13, 0.85, string_ptjet)
            # compare rec for old, new x (w/ overflow, w/o overflow)
            make_plot(
                "efficiency_pr_rec_%d" % ibin2,
                path=self.d_resultsallpdata,
                colours=[get_colour(i) for i in (0, 1, 3, 5, 6)],
                markers=[get_marker(i) for i in (0, 1, 3, 5, 6)],
                list_obj=[
                ...
                title="matched;#it{p}_{T}^{%s} (GeV/#it{c});counts"
                % self.p_latexnhadron,
                leg_pos=[0.55, 0.72, 0.8, 0.86],
                margins_y=[0.05, 0.25],
                logscale="y",
            ...
            make_plot(
                ...
                title="generator level;#it{p}_{T}^{%s} (GeV/#it{c});counts"
                % self.p_latexnhadron,
                leg_pos=[0.55, 0.72, 0.8, 0.86],
                margins_y=[0.05, 0.25],
                logscale="y",
            ...
            make_plot(
                ...
                title="reconstruction level efficiency;#it{p}_{T}^{%s} (GeV/#it{c});efficiency"
                % self.p_latexnhadron,
                leg_pos=[0.55, 0.72, 0.8, 0.86],
                margins_y=[0, 0.25],
            )
            make_plot(
                "efficiency_pr_effgen_%d" % ibin2,
                path=self.d_resultsallpdata,
                colours=[get_colour(i) for i in (0, 5, 6)],
                markers=[get_marker(i) for i in (0, 5, 6)],
                list_obj=[
                    list_ptcand_eff_old[ibin2],
                ...
                title="generator level efficiency;#it{p}_{T}^{%s} (GeV/#it{c});efficiency"
                % self.p_latexnhadron,
                leg_pos=[0.55, 0.72, 0.8, 0.86],
                margins_y=[0, 0.25],
            )
        ...
        make_plot(
            "efficiency_pr_eff_diff",
            path=self.d_resultsallpdata,
            list_obj=list_eff_diff,
            labels_obj=list_eff_diff_labels,
            title="correction of efficiency calculation (rec. level);#it{p}_{T}^{%s} (GeV/#it{c});error = (old #minus new)/new (%%)"
            % self.p_latexnhadron,
            leg_pos=[0.1, 0.62, 0.4, 0.77],
            margins_y=[0.05, 0.05],
        )
        make_plot(
            "efficiency_pr_effgen_diff",
            path=self.d_resultsallpdata,
            list_obj=list_effgen_diff,
            labels_obj=list_effgen_diff_labels,
            title="correction of efficiency calculation (gen. level);#it{p}_{T}^{%s} (GeV/#it{c});error = (old #minus new)/new (%%)"
            % self.p_latexnhadron,
            leg_pos=[0.55, 0.15, 0.85, 0.3],
            margins_y=[0.05, 0.05],
        )
        ...
        setup_canvas(cEffFD)
        legeffFD = TLegend(0.13, 0.65, 0.5, 0.88)
    ...
    def sideband_sub(self):
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            for ipt in range(self.p_nptfinbins):
                ...
                hzsig = hzvsmass.ProjectionY(
                    "hzsig" + suffix, binmasslow2sig, binmasshigh2sig, "e"
                )
                hzbkgleft = hzvsmass.ProjectionY(
                    "hzbkgleft" + suffix, binmasslow9sig, binmasslow4sig, "e"
                )
                hzbkgright = hzvsmass.ProjectionY(
                    "hzbkgright" + suffix, binmasshigh4sig, binmasshigh9sig, "e"
                )
                ...
                if self.sidebandleftonly is False:
                    area_scale_denominator += bkg_fit.Integral(
                        masshigh4sig, masshigh9sig
                    )
                ...
                csblr = TCanvas(
                    "csblr" + suffix, "The Sideband Left-Right Canvas" + suffix
                )
                setup_canvas(csblr)
                ...
                csblr.cd(1)
                legsigbkgsblr = TLegend(0.18, 0.7, 0.45, 0.85)
                ...
                margin_up = 0.3
                margin_down = 0.05
                hzbkgleft.GetYaxis().SetRangeUser(
                    *get_plot_range(y_min_h, y_max_h, margin_down, margin_up)
                )
                hzbkgleft.SetTitle("")
                ...
                if int_n > 0 and int_d > 0:
                    hzbkgratio.Scale(1.0 / int_n)
                    hzbkgratio_d.Scale(1.0 / int_d)
                    hzbkgratio.Divide(hzbkgratio_d)
                    line = TLine(
                        round(self.lvarshape_binmin_reco[0], 2),
                        1,
                    ...
                    margin_up = 0.1
                    margin_down = 0.1
                    hzbkgratio.GetYaxis().SetRangeUser(
                        *get_plot_range(y_min_h, y_max_h, margin_down, margin_up)
                    )
                ...
                csigbkgsubz = TCanvas(
                    "csigbkgsubz" + suffix, "The Side-Band Canvas" + suffix
                )
                setup_canvas(csigbkgsubz)
                legsigbkgsubz = TLegend(0.15, 0.70, 0.35, 0.85)
                ...
                y_min_h, y_max_h = get_y_window_his(
                    [hzsig, hzbkg_scaled, hzsub_noteffscaled]
                )
                y_margin_up = 0.35
                y_margin_down = 0.05
                y_min_0 = min(
                    [h.GetMinimum(0) for h in [hzsig, hzbkg_scaled, hzsub_noteffscaled]]
                )
                if logscale and y_min_h <= 0:
                    y_min_h = y_min_0
                    if y_max_h <= 0:
                        logscale = False
                hzsig.GetYaxis().SetRangeUser(
                    *get_plot_range(
                        y_min_h, y_max_h, y_margin_down, y_margin_up, logscale
                    )
                )
                hzsig.GetXaxis().SetRangeUser(
                    round(self.lvarshape_binmin_reco[0], 2),
                    round(self.lvarshape_binmax_reco[-1], 2),
                )
                hzsig.SetTitle("")
                ...
                latex3 = TLatex(
                    0.42,
                    0.67,
                    "with %s, %g #leq #it{p}_{T, %s} < %g GeV/#it{c}"
                    % (
                        self.p_latexnhadron,
                        self.lpt_finbinmin[ipt],
                        self.p_latexnhadron,
                ...
                if logscale:
                    csigbkgsubz.SetLogy()
                csigbkgsubz.SaveAs(
                    "%s/sideband_sub_%s.eps" % (self.d_resultsallpdata, suffix_plot)
                )
                ...
                if ibin2 in [1] and ipt in [4, 5]:
                    ...
                    if self.shape == "zg":
                        leg_pos = [0.15, 0.15, 0.30, 0.30]
                    elif self.shape == "rg":
                        leg_pos = [0.65, 0.12, 0.85, 0.27]
                    elif self.shape == "nsd":
                        leg_pos = [0.35, 0.12, 0.55, 0.27]
                    else:
                        leg_pos = [0.68, 0.72, 0.85, 0.85]
                    ...
                    y_margin_up = 0.4
                    y_margin_down = 0.05
                    c_sb_sub, list_obj_new = make_plot(
                        "c_sb_sub_" + suffix,
                        size=self.size_can,
                        list_obj=list_obj,
                        labels_obj=labels_obj,
                    ...
                    c_sb_sub.SaveAs(
                        "%s/%s_sb_sub_%s.pdf"
                        % (self.d_resultsallpdata, self.shape, suffix_plot)
                    )
            ...
            cz = TCanvas(
                "cz" + suffix, "The Efficiency Corrected Signal Yield Canvas" + suffix
            )
            setup_canvas(cz)
            ...
            draw_latex(latex)
            cz.SaveAs(
                "%s/sideband_sub_effcorr_ptint_%s.eps"
            ...
            for zbins in range(self.p_nbinshape_reco):
                hzvsjetpt.SetBinContent(
                    zbins + 1, ibin2 + 1, hz.GetBinContent(zbins + 1)
                )
            ...
            for ishape in range(self.p_nbinshape_reco):
                ...
                for ipt in range(self.p_nptfinbins):
                    ...
                    if (
                        frac_sig < 1 - 1e-7 and frac_unc < 1 - 1e-7
                    ):  # avoid numerical errors
                        improve_test = (
                            frac_sig + frac_unc / frac_sig
                        )  # The bin improves the rel. stat. unc. if improve_test < 2.
                        himptest.SetBinContent(
                            ishape + 1, ipt + 1, 1 if improve_test < 2 else 2
                        )
                        ...
                        rel_unc_diff_rel_incl = (
                            rel_unc_tot - rel_unc_tot_without
                        ) / rel_unc_tot_without  # effect of including the bin relative to not including it
                        rel_unc_diff_rel_excl = (
                            rel_unc_tot_without - rel_unc_tot
                        ) / rel_unc_tot  # effect of excluding the bin relative to keeping it
                        # rel_unc_diff_rel_excl = 1 / (1 + rel_unc_diff_rel_incl) - 1
                        himpincl.SetBinContent(
                            ishape + 1, ipt + 1, 100 * rel_unc_diff_rel_incl
                        )
                        himpexcl.SetBinContent(
                            ishape + 1, ipt + 1, 100 * rel_unc_diff_rel_excl
                        )
            ...
            hrelsig.SetTitle(
                "relative contribution to the signal;%s;%s;fraction #it{f}_{#it{S}} (%%)"
                % (self.v_varshape_latex, "%s (GeV/#it{c})" % self.v_pth_latex)
            )
            ...
            crelsig.SaveAs(
                "%s/sideband_relsig_%s.eps" % (self.d_resultsallpdata, suffix)
            )
            ...
            hrelunc.SetTitle(
                "relative contribution to the stat. unc. squared;%s;%s;fraction #it{f}_{#it{#sigma}^{2}} (%%)"
                % (self.v_varshape_latex, "%s (GeV/#it{c})" % self.v_pth_latex)
            )
            ...
            draw_latex(latex, textsize=0.04)
            crelunc.SaveAs(
                "%s/sideband_relunc_%s.eps" % (self.d_resultsallpdata, suffix)
            )
            ...
            hworth.SetTitle(
                "worth = (rel. signal contrib.)/(rel. unc. contrib.);%s;%s;worth #it{w} = #it{f}_{#it{S}}/#it{f}_{#it{#sigma}^{2}}"
                % (self.v_varshape_latex, "%s (GeV/#it{c})" % self.v_pth_latex)
            )
            ...
            himpincl.SetTitle(
                "inclusion impact on the rel. stat. unc.;%s;%s;relative effect (%%)"
                % (self.v_varshape_latex, "%s (GeV/#it{c})" % self.v_pth_latex)
            )
            himpincl.SetTitleSize(0.05, "Z")
            himpincl.SetTitleOffset(1.0, "Z")
            himpincl.GetZaxis().SetRangeUser(
                himpincl.GetMinimum(), himpincl.GetMaximum()
            )
            ...
            draw_latex(latex, textsize=0.04)
            cimpincl.SaveAs(
                "%s/sideband_impincl_%s.eps" % (self.d_resultsallpdata, suffix)
            )
            ...
            himpexcl.SetTitle(
                "exclusion impact on the rel. stat. unc.;%s;%s;relative effect (%%)"
                % (self.v_varshape_latex, "%s (GeV/#it{c})" % self.v_pth_latex)
            )
            himpexcl.SetTitleSize(0.05, "Z")
            himpexcl.SetTitleOffset(1.0, "Z")
            himpexcl.GetZaxis().SetRangeUser(
                himpexcl.GetMinimum(), himpexcl.GetMaximum()
            )
            ...
            draw_latex(latex, textsize=0.04)
            cimpexcl.SaveAs(
                "%s/sideband_impexcl_%s.eps" % (self.d_resultsallpdata, suffix)
            )
            ...
            himptest.SetTitle(
                "Bin improves the rel. stat. unc. if (test value = 1).;%s;%s;test value"
                % (self.v_varshape_latex, "%s (GeV/#it{c})" % self.v_pth_latex)
            )
            ...
            draw_latex(latex, textsize=0.04)
            cimptest.SaveAs(
                "%s/sideband_imptest_%s.eps" % (self.d_resultsallpdata, suffix)
            )
    ...
    def feeddown(self):
        ...
        if not input_data:
            self.logger.fatal(
                make_message_notfound("simulated yields", self.powheg_path_nonprompt)
            )
        ...
        hzvsjetpt_reco_nocuts = feeddown_input_file.Get(
        ...
        creco_eff = TCanvas(
            "creco_eff_nonprompt ", "reco efficiency applied to feedown"
        )
        ...
        his_response_fd = (
            response_matrix.Hresponse()
        )  # linearised response matrix as a TH2
        ...
        his_response_fd.SetXTitle(
            "(#it{p}_{T, jet}^{rec}, %s^{rec}) bin" % self.v_varshape_latex
        )
        his_response_fd.SetYTitle(
            "(#it{p}_{T, jet}^{gen}, %s^{gen}) bin" % self.v_varshape_latex
        )
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            hz_genvsreco_list.append(
                feeddown_input_file.Get("hz_genvsreco_nonprompt" + suffix)
            )

            cz_genvsreco = TCanvas(
                "cz_genvsreco_nonprompt" + suffix, "response matrix 2D projection"
            )
            setup_canvas(cz_genvsreco)
            cz_genvsreco.SetRightMargin(0.13)
            cz_genvsreco.SetLogz()
            setup_histogram(hz_genvsreco_list[ibin2])
            hz_genvsreco_list[ibin2].GetZaxis().SetRangeUser(
                hz_genvsreco_list[ibin2].GetMinimum(0),
                hz_genvsreco_list[ibin2].GetMaximum(),
            )
        ...
        for ibinshape in range(self.p_nbinshape_reco):
            ...
            cjetpt_genvsreco = TCanvas(
                "cjetpt_genvsreco_nonprompt" + suffix, "response matrix 2D projection"
            )
            ...
            hjetpt_genvsreco_list[ibinshape].GetZaxis().SetRangeUser(
                hjetpt_genvsreco_list[ibinshape].GetMinimum(0),
                hjetpt_genvsreco_list[ibinshape].GetMaximum(),
            )
            ...
            hjetpt_genvsreco_list[ibinshape].SetXTitle(
                "#it{p}_{T, jet}^{gen} (GeV/#it{c})"
            )
            hjetpt_genvsreco_list[ibinshape].SetYTitle(
                "#it{p}_{T, jet}^{rec} (GeV/#it{c})"
            )
            hjetpt_genvsreco_list[ibinshape].Draw("colz")
            cjetpt_genvsreco.SaveAs(
                "%s/response_fd_%s_%s.eps"
        ...
        hjetpt_genvsreco_full = feeddown_input_file.Get(
            "hjetpt_genvsreco_full_nonprompt"
        )

        cz_genvsreco = TCanvas(
            "cz_genvsreco_full_nonprompt", "response matrix 2D projection"
        )
        setup_canvas(cz_genvsreco)
        cz_genvsreco.SetRightMargin(0.13)
        cz_genvsreco.SetLogz()
        setup_histogram(hz_genvsreco_full)
        hz_genvsreco_full.GetZaxis().SetRangeUser(
            hz_genvsreco_full.GetMinimum(0), hz_genvsreco_full.GetMaximum()
        )
        ...
        cz_genvsreco.SaveAs(
            "%s/response_fd_%s_full.eps"
            % (self.d_resultsallpdata, self.v_varshape_binning)
        )

        cjetpt_genvsreco = TCanvas(
            "cjetpt_genvsreco_full_nonprompt", "response matrix 2D projection"
        )
        setup_canvas(cjetpt_genvsreco)
        cjetpt_genvsreco.SetRightMargin(0.13)
        cjetpt_genvsreco.SetLogz()
        setup_histogram(hjetpt_genvsreco_full)
        hjetpt_genvsreco_full.GetZaxis().SetRangeUser(
            hjetpt_genvsreco_full.GetMinimum(0), hjetpt_genvsreco_full.GetMaximum()
        )
        ...
        cjetpt_genvsreco.SaveAs(
            "%s/response_fd_%s_full.eps" % (self.d_resultsallpdata, self.v_var2_binning)
        )

        hz_genvsreco_full_real = feeddown_input_file.Get(
            "hz_genvsreco_full_nonprompt_real"
        )
        hjetpt_genvsreco_full_real = feeddown_input_file.Get(
            "hjetpt_genvsreco_full_nonprompt_real"
        )

        cz_genvsreco_real = TCanvas(
            "cz_genvsreco_full_nonprompt_real", "response matrix 2D projection_real"
        )
        ...
        hz_genvsreco_full_real.GetZaxis().SetRangeUser(
            hz_genvsreco_full_real.GetMinimum(0), hz_genvsreco_full_real.GetMaximum()
        )
        ...
        cz_genvsreco_real.SaveAs(
            "%s/response_fd_%s_full_real.eps"
            % (self.d_resultsallpdata, self.v_varshape_binning)
        )

        cjetpt_genvsreco_real = TCanvas(
            "cjetpt_genvsreco_full_nonprompt_real", "response matrix 2D projection_real"
        )
        ...
        hjetpt_genvsreco_full_real.GetZaxis().SetRangeUser(
            hjetpt_genvsreco_full_real.GetMinimum(0),
            hjetpt_genvsreco_full_real.GetMaximum(),
        )
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            ceff = TCanvas(
                "ceff " + suffix, "prompt and non-prompt efficiencies" + suffix
            )
            setup_canvas(ceff)
            leg_eff = TLegend(0.7, 0.15, 0.85, 0.25, "")
            ...
            y_min_h, y_max_h = get_y_window_his(
                [heff_pr_list[ibin2], heff_fd_list[ibin2]]
            )
            y_min_h = 0
            y_margin_up = 0.05
            y_margin_down = 0
            bin_pt_max = min(
                ...
                heff_pr_list[ibin2]
                .GetXaxis()
                .FindBin(self.lvar2_binmax_gen[ibin2] - 0.01),
            )
            ...
            latex3 = TLatex(
                ...
                0.73,
                (
                    "%s #rightarrow %s (and charge conj.)"
                    % (self.p_latexnhadron, self.p_latexndecay)
                ),
            )
            draw_latex(latex3)
            latex4 = TLatex(
                0.15, 0.68, "in charged jets, anti-#it{k}_{T}, #it{R} = 0.4"
            )
            draw_latex(latex4)
            latex5 = TLatex(
                0.15,
            ...
            draw_latex(latex6)
            ceff.SaveAs(
                "%s/efficiency_pr_fd_%s.eps" % (self.d_resultsallpdata, suffix_plot)
            )

            # preliminary figure
            if ibin2 in [1]:
                leg_pos = [0.7, 0.15, 0.85, 0.30]
                ...
                y_margin_up = 0.3
                y_margin_down = 0.05
                c_eff_both, list_obj_new = make_plot(
                    "c_eff_both_" + suffix,
                    size=self.size_can,
                    list_obj=list_obj,
                    labels_obj=labels_obj,
                    ...
                    leg_pos=leg_pos,
                    margins_y=[y_margin_down, y_margin_up],
                    margins_c=[0.12, 0.13, 0.05, 0.03],
                    title=";#it{p}_{T}^{%s} (GeV/#it{c});reconstruction efficiency"
                    % self.p_latexnhadron,
                )
                list_obj_new[0].SetTextSize(self.fontsize)
                # Draw LaTeX
                ...
                c_eff_both.Update()
                c_eff_both.SaveAs(
                    "%s/%s_eff_pr_fd_%s.pdf"
                    % (self.d_resultsallpdata, self.shape, suffix_plot)
                )

        # plot relative jet pt shift

        cjetpt_fracdiff = TCanvas(
            "cjetpt_fracdiff ", "non-prompt jetpt response fractional differences"
        )
        setup_canvas(cjetpt_fracdiff)
        cjetpt_fracdiff.SetLogy()
        leg_jetpt_fracdiff = TLegend(
            0.15, 0.5, 0.25, 0.8, "#it{p}_{T, jet}^{gen} (GeV/#it{c})"
        )
        setup_legend(leg_jetpt_fracdiff)
        for ibin2 in range(self.p_nbin2_gen):
            setup_histogram(
                hjetpt_fracdiff_list[ibin2], get_colour(ibin2), get_marker(ibin2)
            )
            leg_jetpt_fracdiff.AddEntry(
                hjetpt_fracdiff_list[ibin2],
                "%g#minus%g"
            ...
            if ibin2 == 0:
                ...
                hjetpt_fracdiff_list[ibin2].GetYaxis().SetRangeUser(
                    *get_plot_range(y_min_0, y_max_h, y_margin_down, y_margin_up, True)
                )
            hjetpt_fracdiff_list[ibin2].Draw("same")
        leg_jetpt_fracdiff.Draw("same")
        cjetpt_fracdiff.SaveAs(
            "%s/response_fd_reldiff_%s.eps"
            % (self.d_resultsallpdata, self.v_var2_binning)
        )
        ...
        for ibinshape in range(self.p_nbinshape_gen):
            ...
            hz_fracdiff_list.append(
                feeddown_input_file.Get("hz_fracdiff_nonprompt" + suffix)
            )

        cz_fracdiff = TCanvas(
            "cz_fracdiff ", "non-prompt z response fractional differences"
        )
        setup_canvas(cz_fracdiff)
        cz_fracdiff.SetLogy()
        leg_z_fracdiff = TLegend(
            0.15, 0.5, 0.25, 0.8, "%s^{gen}" % self.v_varshape_latex
        )
        setup_legend(leg_z_fracdiff)
        for ibinshape in range(self.p_nbinshape_gen):
            setup_histogram(
                hz_fracdiff_list[ibinshape],
                get_colour(ibinshape),
                get_marker(ibinshape),
            )
            leg_z_fracdiff.AddEntry(
                hz_fracdiff_list[ibinshape],
                "%g#minus%g"
                % (
                    self.lvarshape_binmin_gen[ibinshape],
            ...
            if ibinshape == 0:
                ...
                hz_fracdiff_list[ibinshape].SetXTitle(
                    "(%s^{rec} #minus %s^{gen})/%s^{gen}"
                    % (
                        self.v_varshape_latex,
                        self.v_varshape_latex,
                        self.v_varshape_latex,
                ...
                y_margin_up = 0.15
                y_margin_down = 0.05
                hz_fracdiff_list[ibinshape].GetYaxis().SetRangeUser(
                    *get_plot_range(y_min_0, y_max_h, y_margin_down, y_margin_up, True)
                )
        ...
        for ipt in range(self.p_nptfinbins):
            ...
            for ibin2 in range(self.p_nbin2_gen):
                for ibinshape in range(self.p_nbinshape_gen):
                    ...
                    if (
                        heff_pr_list[ibin2].GetBinContent(ipt + 1) == 0
                        or heff_fd_list[ibin2].GetBinContent(ipt + 1) == 0
                    ):
                        input_data_zvsjetpt_list[ipt].SetBinContent(
                            ibinshape + 1, ibin2 + 1, 0.0
                        )
                    else:
                        input_data_zvsjetpt_list[ipt].SetBinContent(
                            ibinshape + 1,
                            ibin2 + 1,
                            input_data_zvsjetpt_list[ipt].GetBinContent(
                                ibinshape + 1, ibin2 + 1
                            )
                            * (
            ...
            # sum up pt bins
            if ipt == 0:
                input_data_scaled = input_data_zvsjetpt_list[ipt].Clone(
                    "input_data_scaled"
                )
            else:
        ...
        input_data_scaled.Scale(
            self.p_nevents * self.branching_ratio / self.xsection_inel
        )
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            folded_z_list.append(
                folded.ProjectionX(
                    "folded_z_nonprompt_" + suffix, ibin2 + 1, ibin2 + 1, "e"
                )
            )
            input_data_scaled_z_list.append(
                input_data_scaled.ProjectionX(
                    "Powheg_scaled_nonprompt_" + suffix,
            ...
            leg_fd_fold = TLegend(0.15, 0.75, 0.4, 0.85, "")
            setup_legend(leg_fd_fold)
            setup_histogram(
                input_data_scaled_z_list[ibin2], get_colour(1), get_marker(0)
            )
            leg_fd_fold.AddEntry(
                input_data_scaled_z_list[ibin2], "POWHEG, eff. scaled", "P"
            )
            y_min_h, y_max_h = get_y_window_his(
                [input_data_scaled_z_list[ibin2], folded_z_list[ibin2]]
            )
            y_margin_up = 0.2
            y_margin_down = 0.05
            input_data_scaled_z_list[ibin2].GetYaxis().SetRangeUser(
                *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
            )
            ...
            leg_fd_fold.AddEntry(
                folded_z_list[ibin2], "POWHEG, eff. scaled, folded", "P"
            )
            ...
            draw_latex(latex)
            c_fd_fold.SaveAs(
                "%s/feeddown_folded_%s.eps" % (self.d_resultsallpdata, suffix_plot)
            )
        ...
        sideband_input_data_subtracted = sideband_input_data.Clone(
        ...
        for ibin2 in range(self.p_nbin2_reco):
            for ibinshape in range(self.p_nbinshape_reco):
                if (
                    sideband_input_data_subtracted.GetBinContent(
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            sideband_input_data_z.append(
                sideband_input_data.ProjectionX(
                    "sideband_input_data_z" + suffix, ibin2 + 1, ibin2 + 1, "e"
                )
            )
            sideband_input_data_subtracted_z.append(
                sideband_input_data_subtracted.ProjectionX(
                    "sideband_input_data_subtracted_z" + suffix,
                    ibin2 + 1,
                    ibin2 + 1,
                    "e",
                )
            ...
            if logscale:
                cfeeddown.SetLogy()
            leg_feeddown = TLegend(0.18, 0.70, 0.35, 0.85)
            setup_legend(leg_feeddown)
            setup_histogram(sideband_input_data_z[ibin2], get_colour(1), get_marker(0))
            leg_feeddown.AddEntry(
                sideband_input_data_z[ibin2], "prompt & non-prompt", "P"
            )
            l_his = [
                sideband_input_data_z[ibin2],
                sideband_input_data_subtracted_z[ibin2],
                folded_z_list[ibin2],
            ]
            ...
            y_margin_up = 0.27
            y_margin_down = 0.05
            sideband_input_data_z[ibin2].GetYaxis().SetRangeUser(
                *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up, logscale)
            )
            ...
            folded_z_list[ibin2].Draw("same")
            setup_histogram(
                sideband_input_data_subtracted_z[ibin2], get_colour(3), get_marker(2)
            )
            leg_feeddown.AddEntry(
                sideband_input_data_subtracted_z[ibin2], "subtracted (prompt)", "P"
            )
            ...
            draw_latex(latex)
            cfeeddown.SaveAs(
                "%s/feeddown_subtraction_%s.eps" % (self.d_resultsallpdata, suffix_plot)
            )

            feeddown_fraction = folded_z_list[ibin2].Clone("feeddown_fraction" + suffix)
            feeddown_fraction_denominator = sideband_input_data_z[ibin2].Clone(
                "feeddown_denominator" + suffix
            )
            feeddown_fraction.Divide(feeddown_fraction_denominator)
            feeddown_fraction.Write()

            cfeeddown_fraction = TCanvas(
                "cfeeddown_fraction" + suffix, "cfeeddown_fraction" + suffix
            )
            setup_canvas(cfeeddown_fraction)
            cfeeddown_fraction.SetLeftMargin(0.13)
            leg_fd_fraction = TLegend(0.18, 0.8, 0.35, 0.85)
            ...
            y_margin_up = 0.15
            y_margin_down = 0.05
            feeddown_fraction.GetYaxis().SetRangeUser(
                *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
            )
            ...
            draw_latex(latex)
            cfeeddown_fraction.SaveAs(
                "%s/feeddown_fraction_%s.eps" % (self.d_resultsallpdata, suffix_plot)
            )
        ...
        sideband_input_data_subtracted.SetYTitle(
            "%s (GeV/#it{c})" % self.p_latexbin2var
        )
    ...
    def unfolding(self):
        ...
        if not input_data:
            self.logger.fatal(
                make_message_notfound(
                    "sideband_input_data_subtracted", self.file_feeddown
                )
            )
        ...
        if not input_mc_det:
            self.logger.fatal(
                make_message_notfound("input_closure_reco", self.n_fileresp)
            )

        stat_unfolding = input_data.Integral()
        stat_closure = input_mc_det.Integral()
        print(
            "Unfolding: data statistics: %g, closure statistics: %g, ratio: %g"
            % (stat_unfolding, stat_closure, stat_unfolding / stat_closure)
        )
        ...
        if not input_mc_gen:
            self.logger.fatal(
                make_message_notfound("hzvsjetpt_gen_unmatched", self.n_fileresp)
            )
        # rec. level cuts only applied
        ...
        input_data_jetpt = input_data.ProjectionY(
            "input_data_jetpt", 1, self.p_nbinshape_reco, "e"
        )
        ...
        if not input_powheg:
            self.logger.fatal(
                make_message_notfound("simulated yields", self.powheg_path_prompt)
            )
        ...
        if not equal_binning_lists(input_powheg, list_y=self.var2ranges_gen):
            self.logger.fatal("Error: Incorrect binning in y.")
        # Ensure correct binning: x - shape, y - jet pt
        if not equal_binning_lists(
            input_powheg_xsection, list_x=self.varshaperanges_gen
        ):
            self.logger.fatal("Error: Incorrect binning in x.")
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            input_mc_gen_z.append(
                input_mc_gen.ProjectionX(
                    "input_mc_gen_z" + suffix, ibin2 + 1, ibin2 + 1, "e"
                )
            )
            input_mc_gen_z[ibin2].Scale(
                1.0
                / input_mc_gen_z[ibin2].Integral(
                    bin_int_first,
                    input_mc_gen_z[ibin2].FindBin(self.lvarshape_binmin_reco[-1]),
                ),
                "width",
            )
            input_powheg_z.append(
                input_powheg.ProjectionX(
                    "input_powheg_z" + suffix, ibin2 + 1, ibin2 + 1, "e"
                )
            )
            input_powheg_z[ibin2].Scale(
                1.0
                / input_powheg_z[ibin2].Integral(
                    bin_int_first,
                    input_powheg_z[ibin2].FindBin(self.lvarshape_binmin_reco[-1]),
                ),
                "width",
            )
            input_powheg_xsection_z.append(
                input_powheg_xsection.ProjectionX(
                    "input_powheg_xsection_z" + suffix, ibin2 + 1, ibin2 + 1, "e"
                )
            )
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            input_data_z.append(
                input_data.ProjectionX(
                    "input_data_z" + suffix, ibin2 + 1, ibin2 + 1, "e"
                )
            )

            # compare shapes of distributions of reconstructed jets that pass rec. vs. gen. level cuts

            mc_reco_matched_z.append(
                mc_reco_matched.ProjectionX(
                    "mc_reco_matched_z" + suffix, ibin2 + 1, ibin2 + 1, "e"
                )
            )
            mc_reco_matched_z[ibin2].Scale(
                1.0 / mc_reco_matched_z[ibin2].Integral(bin_int_first, -1)
            )
            mc_gen_matched_z.append(
                mc_gen_matched.ProjectionX(
                    "mc_det_matched_z" + suffix,
                    mc_gen_matched.GetYaxis().FindBin(self.lvar2_binmin_reco[ibin2]),
                    mc_gen_matched.GetYaxis().FindBin(self.lvar2_binmin_reco[ibin2]),
                    "e",
                )
            )
            mc_gen_matched_z[ibin2].Scale(
                1.0 / mc_gen_matched_z[ibin2].Integral(bin_int_first, -1)
            )
            mc_reco_gen_matched_z_ratio.append(
                mc_reco_matched_z[ibin2].Clone(
                    "input_mc_reco_gen_matched_z_ratio" + suffix
                )
            )
            mc_reco_gen_matched_z_ratio[ibin2].Divide(mc_gen_matched_z[ibin2])

            c_mc_reco_gen_matched_z_ratio = TCanvas(
                "c_mc_reco_gen_matched_z_ratio " + suffix, "Reco/Gen Ratio"
            )
            ...
            y_margin_up = 0.15
            y_margin_down = 0.05
            mc_reco_gen_matched_z_ratio[ibin2].GetYaxis().SetRangeUser(
                *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
            )
            ...
            draw_latex(latex)
            c_mc_reco_gen_matched_z_ratio.SaveAs(
                "%s/reco_gen_matched_%s_ratio_%s.eps"
                % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot)
            )

            c_mc_reco_gen_matched_z = TCanvas(
                "c_mc_reco_gen_matched_z " + suffix, "Reco vs Gen"
            )
            setup_canvas(c_mc_reco_gen_matched_z)
            c_mc_reco_gen_matched_z.SetLeftMargin(0.13)
            leg_mc_reco_gen_matched_z = TLegend(0.6, 0.8, 0.8, 0.85, "")
            setup_legend(leg_mc_reco_gen_matched_z)
            setup_histogram(mc_reco_matched_z[ibin2], get_colour(1), get_marker(0))
            leg_mc_reco_gen_matched_z.AddEntry(
                mc_reco_matched_z[ibin2], "reconstructed", "P"
            )
            y_min_h, y_max_h = get_y_window_his(
                [mc_reco_matched_z[ibin2], mc_gen_matched_z[ibin2]]
            )
            y_margin_up = 0.15
            y_margin_down = 0.05
            mc_reco_matched_z[ibin2].GetYaxis().SetRangeUser(
                *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
            )
            ...
            mc_reco_matched_z[ibin2].GetXaxis().SetRangeUser(
                round(self.lvarshape_binmin_reco[0], 2),
                round(self.lvarshape_binmax_reco[-1], 2),
            )
            ...
            leg_mc_reco_gen_matched_z.AddEntry(
                mc_gen_matched_z[ibin2], "generated", "P"
            )
            ...
            draw_latex(latex)
            c_mc_reco_gen_matched_z.SaveAs(
                "%s/reco_gen_matched_%s_%s.eps"
                % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot)
            )
            ...
            cz_genvsreco = TCanvas(
                "cz_genvsreco_" + suffix, "response matrix 2D projection"
            )
            setup_canvas(cz_genvsreco)
            cz_genvsreco.SetRightMargin(0.13)
            cz_genvsreco.SetLogz()
        ...
        for ibinshape in range(self.p_nbinshape_reco):
            ...
            hjetpt_genvsreco_list.append(
                unfolding_input_file.Get("hjetpt_genvsreco" + suffix)
            )

            cjetpt_genvsreco = TCanvas(
                "cjetpt_genvsreco" + suffix, "response matrix 2D projection" + suffix
            )
            setup_canvas(cjetpt_genvsreco)
            cjetpt_genvsreco.SetRightMargin(0.13)
            cjetpt_genvsreco.SetLogz()
        ...
        his_response_pr.SetXTitle(
            "(#it{p}_{T, jet}^{rec}, %s^{rec}) bin" % self.v_varshape_latex
        )
        his_response_pr.SetYTitle(
            "(#it{p}_{T, jet}^{gen}, %s^{gen}) bin" % self.v_varshape_latex
        )
        ...
        if not hz_genvsreco_full:
            self.logger.fatal(
                make_message_notfound("hz_genvsreco_full", self.n_fileresp)
            )
        hjetpt_genvsreco_full = unfolding_input_file.Get("hjetpt_genvsreco_full")
        if not hjetpt_genvsreco_full:
            self.logger.fatal(
                make_message_notfound("hjetpt_genvsreco_full", self.n_fileresp)
            )
        ...
        text_pth_full = self.text_pth % (
            ...
            self.p_latexnhadron,
            self.lpt_finbinmax[-1],
            self.p_latexnhadron,
        )
        cz_genvsreco_full = TCanvas(
            "cz_genvsreco_full", "response matrix 2D projection", 800, 800
        )
        ...
        hz_genvsreco_full.SetTitle(
            ";%s^{gen};%s^{rec};self-normalised yield"
            % (self.v_varshape_latex, self.v_varshape_latex)
        )
        hz_genvsreco_full.Draw("colz")
        y_latex = 0.95
        list_latex = []
        ...
        cz_genvsreco_full.SaveAs(
            "%s/response_pr_%s_full.pdf"
            % (self.d_resultsallpdata, self.v_varshape_binning)
        )
        cz_genvsreco_full.SaveAs(
            "%s/%s_resp_pr_full.pdf" % (self.d_resultsallpdata, self.shape)
        )

        cjetpt_genvsreco_full = TCanvas(
            "cjetpt_genvsreco_full", "response matrix 2D projection"
        )
        ...
        hjetpt_genvsreco_full.SetYTitle("#it{p}_{T, jet}^{rec} (GeV/#it{c})")
        hjetpt_genvsreco_full.Draw("colz")
        cjetpt_genvsreco_full.SaveAs(
            "%s/response_pr_%s_full.eps" % (self.d_resultsallpdata, self.v_var2_binning)
        )

        hz_genvsreco_full_real = unfolding_input_file.Get("hz_genvsreco_full_real")
        if not hz_genvsreco_full_real:
            self.logger.fatal(
                make_message_notfound("hz_genvsreco_full_real", self.n_fileresp)
            )
        hjetpt_genvsreco_full_real = unfolding_input_file.Get(
            "hjetpt_genvsreco_full_real"
        )
        if not hjetpt_genvsreco_full_real:
            self.logger.fatal(
                make_message_notfound("hjetpt_genvsreco_full_real", self.n_fileresp)
            )

        hz_genvsreco_full_real.Scale(1.0 / hz_genvsreco_full_real.Integral())

        # preliminary figure
        cz_genvsreco_full_real = TCanvas(
            "cz_genvsreco_full_real", "response matrix 2D projection_real"
        )
        ...
        cz_genvsreco_full_real.SaveAs(
            "%s/response_pr_%s_full_real.pdf"
            % (self.d_resultsallpdata, self.v_varshape_binning)
        )
        cz_genvsreco_full_real.SaveAs(
            "%s/%s_resp_pr_full_real.pdf" % (self.d_resultsallpdata, self.shape)
        )

        cjetpt_genvsreco_full_real = TCanvas(
            "cjetpt_genvsreco_full_real", "response matrix 2D projection_real"
        )
        ...
        hjetpt_genvsreco_full_real.SetYTitle("#it{p}_{T, jet}^{rec} (GeV/#it{c})")
        hjetpt_genvsreco_full_real.Draw("colz")
        cjetpt_genvsreco_full_real.SaveAs(
            "%s/response_pr_%s_full_real.eps"
            % (self.d_resultsallpdata, self.v_var2_binning)
        )
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            ckinematic_eff = TCanvas(
                "ckinematic_eff " + suffix, "Kinematic Eff" + suffix
            )
            ...
            y_margin_up = 0.15
            y_margin_down = 0.05
            kinematic_eff[ibin2].GetYaxis().SetRangeUser(
                *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
            )
            kinematic_eff[ibin2].GetXaxis().SetRangeUser(
                round(self.lvarshape_binmin_reco[0], 2),
                round(self.lvarshape_binmax_reco[-1], 2),
            )
            ...
            draw_latex(latex)
            ckinematic_eff.SaveAs(
                "%s/kineff_pr_gen_%s_%s.eps"
                % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot)
            )
        ...
        y_margin_up = 0.15
        y_margin_down = 0.05
        kinematic_eff_jetpt.GetYaxis().SetRangeUser(
            *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
        )
        kinematic_eff_jetpt.GetXaxis().SetRangeUser(
            round(self.lvar2_binmin_reco[0], 2), round(self.lvar2_binmax_reco[-1], 2)
        )
        ...
        draw_latex(latex)
        ckinematic_eff_jetpt.SaveAs(
            "%s/kineff_pr_gen_%s.eps" % (self.d_resultsallpdata, self.v_var2_binning)
        )
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            hjetpt_fracdiff_list.append(
                unfolding_input_file.Get("hjetpt_fracdiff_prompt" + suffix)
            )

        cjetpt_fracdiff = TCanvas(
            "cjetpt_fracdiff", "prompt jetpt response fractional differences"
        )
        setup_canvas(cjetpt_fracdiff)
        cjetpt_fracdiff.SetLogy()
        leg_jetpt_fracdiff = TLegend(
        ...
        for ibinshape in range(self.p_nbinshape_gen):
            ...
            hz_fracdiff_list.append(
                unfolding_input_file.Get("hz_fracdiff_prompt" + suffix)
            )

        cz_fracdiff = TCanvas("cz_fracdiff", "prompt z response fractional differences")
        setup_canvas(cz_fracdiff)
        cz_fracdiff.SetLogy()
        leg_z_fracdiff = TLegend(0.15, 0.5, 0.25, 0.8, self.v_varshape_latex)
        setup_legend(leg_z_fracdiff)
        for ibinshape in range(self.p_nbinshape_gen):
            setup_histogram(
                hz_fracdiff_list[ibinshape],
        ...
        i_iter_choice = (
            self.choice_iter_unfolding - 1
        )  # list index of the chosen iteration
        for i in range(self.niter_unfolding):
            ...
            if i == i_iter_choice:
                unfolded_zvsjetpt_final = unfolded_zvsjetpt.Clone(
                    "unfolded_zvsjetpt_final"
                )
                ...
                unfolded_zvsjetpt_final.SetYTitle(
                    "%s (GeV/#it{c})" % self.p_latexbin2var
                )
                gStyle.SetPaintTextFormat(".0f")
                unfolded_zvsjetpt_final.Draw("texte")
                cunfolded_output.SaveAs(
                    "%s/unfolded_output.eps" % self.d_resultsallpdata
                )
                gStyle.SetPaintTextFormat("g")
            ...
            for ibin2 in range(self.p_nbin2_gen):
                ...
                unfolded_z = unfolded_zvsjetpt.ProjectionX(
                    "unfolded_z_proj_%d_%s" % (i + 1, suffix), ibin2 + 1, ibin2 + 1, "e"
                )
                unfolded_z_scaled = unfolded_z.Clone(
                    "unfolded_z_scaled_%d_%s" % (i + 1, suffix)
                )
                ...
                unfolded_z_xsection = unfolded_z_scaled.Clone(
                    "unfolded_z_xsection_%d_%s" % (i + 1, suffix)
                )
                unfolded_z_xsection.Scale(
                    self.xsection_inel / (self.p_nevents * self.branching_ratio),
                    "width",
                )

                # normalise by the number of jets

                unfolded_z_scaled.Scale(
                    1.0
                    / unfolded_z_scaled.Integral(
                        bin_int_first,
                        unfolded_z_scaled.FindBin(self.lvarshape_binmin_reco[-1]),
                    ),
                    "width",
                )
                ...
                cunfolded_z = TCanvas(
                    "cunfolded_z_%d_%s" % (i + 1, suffix),
                    "1D output of unfolding" + suffix,
                )
                ...
                y_margin_up = 0.15
                y_margin_down = 0.05
                unfolded_z_scaled.GetYaxis().SetRangeUser(
                    *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
                )
                unfolded_z_scaled.GetXaxis().SetRangeUser(
                    round(self.lvarshape_binmin_reco[0], 2),
                    round(self.lvarshape_binmax_reco[-1], 2),
                )
                unfolded_z_scaled.SetTitle("")
                unfolded_z_scaled.SetXTitle(self.v_varshape_latex)
                unfolded_z_scaled.SetYTitle(
                    "1/#it{N}_{jets} d#it{N}/d%s" % self.v_varshape_latex
                )
            ...
            unfolded_jetpt = unfolded_zvsjetpt.ProjectionY(
                "unfolded_jetpt_%d" % (i + 1), 1, self.p_nbinshape_gen, "e"
            )
            unfolded_jetpt_scaled = unfolded_jetpt.Clone(
                "unfolded_jetpt_scaled_%d" % (i + 1)
            )
            ...
            unfolded_jetpt_scaled.Scale(
                1.0
                / unfolded_jetpt_scaled.Integral(
                    unfolded_jetpt_scaled.FindBin(self.lvar2_binmin_reco[0]),
                    unfolded_jetpt_scaled.FindBin(self.lvar2_binmin_reco[-1]),
                ),
                "width",
            )
            ...
            cunfolded_jetpt = TCanvas(
                "cunfolded_jetpt_%s" % (i + 1), "1D output of unfolding"
            )
            ...
            y_margin_up = 0.15
            y_margin_down = 0.05
            unfolded_jetpt_scaled.GetYaxis().SetRangeUser(
                *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up, True)
            )
            unfolded_jetpt_scaled.GetXaxis().SetRangeUser(
            ...
            unfolded_jetpt_scaled.SetYTitle(
                "1/#it{N}_{jets} d#it{N}/d%s (#it{c}/GeV)" % self.p_latexbin2var
            )
            ...
            draw_latex(latex2)
            cunfolded_jetpt.SaveAs(
                "%s/unfolded_%s_%d.eps"
                % (self.d_resultsallpdata, self.v_var2_binning, i + 1)
            )
            ...
            for ibin2 in range(self.p_nbin2_reco):
                ...
                refolded_z = refolded.ProjectionX(
                    "refolded_z", ibin2 + 1, ibin2 + 1, "e"
                )
                refolding_test = input_data_z[ibin2].Clone(
                    "refolding_test_%d_%s" % (i + 1, suffix)
                )
                refolding_test.Divide(refolded_z)
                refolding_test_list_iter.append(refolding_test)
                cfolded_z = TCanvas(
                    "cfolded_z_%d_%s" % (i + 1, suffix), "1D output of folding" + suffix
                )
            ...
            refolded_jetpt = refolded.ProjectionY(
                "refolded_jetpt", 1, self.p_nbinshape_gen, "e"
            )
            ...
            cfolded_jetpt = TCanvas(
                "cfolded_jetpt_%d" % (i + 1), "1D output of folding"
            )
            setup_canvas(cfolded_jetpt)
            setup_histogram(refolding_test_jetpt, get_colour(1))
            line = TLine(
                round(self.lvar2_binmin_gen[0], 2),
                1,
                round(self.lvar2_binmax_gen[-1], 2),
                1,
            )
            ...
            latex = TLatex(
                ...
                "%g #leq %s < %g"
                % (
                    round(self.lvarshape_binmin_gen[0], 2),
                    self.v_varshape_latex,
            ...
            latex2 = TLatex(0.5, 0.82, "iteration %d" % (i + 1))
            draw_latex(latex2)
            cfolded_jetpt.SaveAs(
                "%s/refolding_%s_%d.eps"
                % (self.d_resultsallpdata, self.v_var2_binning, i + 1)
            )
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            cconvergence_z = TCanvas(
                "cconvergence_z " + suffix, "1D output of convergence"
            )
            setup_canvas(cconvergence_z)
            cconvergence_z.SetRightMargin(0.2)
            leg_z = TLegend(0.85, 0.45, 0.95, 0.85, "iteration")
            setup_legend(leg_z)
            l_his = [
                unfolded_z_scaled_list[i][ibin2] for i in range(self.niter_unfolding)
            ]
            y_min_h, y_max_h = get_y_window_his(l_his)
            y_margin_up = 0.15
            y_margin_down = 0.05
            for i in range(self.niter_unfolding):
                ...
                if i == 0:
                    unfolded_z_scaled_list[i][ibin2].GetXaxis().SetRangeUser(
                        round(self.lvarshape_binmin_reco[0], 2),
                        round(self.lvarshape_binmax_reco[-1], 2),
                    )
                    unfolded_z_scaled_list[i][ibin2].GetYaxis().SetRangeUser(
                        *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
                    )
                    unfolded_z_scaled_list[i][ibin2].SetTitle("")
                    unfolded_z_scaled_list[i][ibin2].SetXTitle(self.v_varshape_latex)
                    unfolded_z_scaled_list[i][ibin2].SetYTitle(
                        "1/#it{N}_{jets} d#it{N}/d%s" % self.v_varshape_latex
                    )
            ...
            draw_latex(latex)
            cconvergence_z.SaveAs(
                "%s/convergence_%s_%s.eps"
                % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot)
            )
            ...
            leg_input_mc_gen_z = TLegend(0.15, 0.75, 0.45, 0.85)
            setup_legend(leg_input_mc_gen_z)
            setup_histogram(input_mc_gen_z[ibin2], get_colour(2), get_marker(1))
            y_min_h, y_max_h = get_y_window_his(
                [
                    unfolded_z_scaled_list[i_iter_choice][ibin2],
                    input_mc_gen_z[ibin2],
                    input_powheg_z[ibin2],
                ]
            )
            ...
            leg_input_mc_gen_z.AddEntry(
                unfolded_z_scaled_list[i_iter_choice][ibin2], "unfolded data", "P"
            )
            ...
            draw_latex(latex)
            cinput_mc_gen_z.SaveAs(
                "%s/unfolded_vs_mc_%s_%s.eps"
                % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot)
            )
            ...
            cinput_mc_gen_z_xsection = TCanvas(
                "cinput_mc_gen_z_xsection " + suffix, "1D gen pythia z xsection"
            )
            setup_canvas(cinput_mc_gen_z_xsection)
            leg_input_mc_gen_z_xsection = TLegend(0.15, 0.75, 0.45, 0.85)
            setup_legend(leg_input_mc_gen_z_xsection)
            setup_histogram(
                unfolded_z_xsection_list[i_iter_choice][ibin2],
                get_colour(1),
                get_marker(0),
            )
            leg_input_mc_gen_z_xsection.AddEntry(
                unfolded_z_xsection_list[i_iter_choice][ibin2], "unfolded data", "P"
            )
            y_min_h, y_max_h = get_y_window_his(
                [
                    unfolded_z_xsection_list[i_iter_choice][ibin2],
                    input_powheg_xsection_z[ibin2],
                ]
            )
            ...
            y_margin_up = 0.2
            y_margin_down = 0.05
            unfolded_z_xsection_list[i_iter_choice][ibin2].GetYaxis().SetRangeUser(
                *get_plot_range(y_min, y_max, y_margin_down, y_margin_up)
            )
            unfolded_z_xsection_list[i_iter_choice][ibin2].GetXaxis().SetRangeUser(
                round(self.lvarshape_binmin_reco[0], 2),
                round(self.lvarshape_binmax_reco[-1], 2),
            )
            unfolded_z_xsection_list[i_iter_choice][ibin2].SetTitle("")
            unfolded_z_xsection_list[i_iter_choice][ibin2].SetXTitle(
                self.v_varshape_latex
            )
            unfolded_z_xsection_list[i_iter_choice][ibin2].SetYTitle(
                "d#it{#sigma}/d%s (mb)" % self.v_varshape_latex
            )
            unfolded_z_xsection_list[i_iter_choice][ibin2].GetYaxis().SetMaxDigits(3)
            unfolded_z_xsection_list[i_iter_choice][ibin2].Draw()
            setup_histogram(
                input_powheg_xsection_z[ibin2], get_colour(3), get_marker(2)
            )
            leg_input_mc_gen_z_xsection.AddEntry(
                input_powheg_xsection_z[ibin2], "POWHEG + PYTHIA 6", "P"
            )
            ...
            cinput_mc_gen_z_xsection.SaveAs(
                "%s/unfolded_vs_mc_%s_xsection_%s.eps"
                % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot)
            )
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            cconvergence_refolding_z = TCanvas(
                "cconvergence_refolding_z " + suffix,
                "1D output of refolding convergence" + suffix,
            )
            setup_canvas(cconvergence_refolding_z)
            cconvergence_refolding_z.SetRightMargin(0.2)
            leg_refolding_z = TLegend(0.85, 0.45, 0.95, 0.85, "iteration")
            ...
            for i in range(self.niter_unfolding):
                ...
                leg_refolding_z.AddEntry(
                    refolding_test_list[i][ibin2], ("%d" % (i + 1)), "P"
                )
                ...
                if i == 0:
                    ...
                    refolding_test_list[i][ibin2].GetYaxis().SetRangeUser(
                        *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
                    )
            ...
            draw_latex(latex)
            cconvergence_refolding_z.SaveAs(
                "%s/convergence_refolding_%s_%s.eps"
                % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot)
            )

            # compare the result before unfolding and after

            input_data_z_scaled = input_data_z[ibin2].Clone(
                "input_data_z_scaled_%s" % suffix
            )
            input_data_z_scaled.Scale(
                1.0 / input_data_z_scaled.Integral(bin_int_first, -1), "width"
            )
            cunfolded_not_z = TCanvas(
                "cunfolded_not_z " + suffix, "Unfolded vs not Unfolded" + suffix
            )
            setup_canvas(cunfolded_not_z)
            leg_cunfolded_not_z = TLegend(0.15, 0.75, 0.45, 0.85)
            setup_legend(leg_cunfolded_not_z)
            ibin_jetpt = (
                input_mc_gen.GetYaxis().FindBin(self.lvar2_binmin_reco[ibin2]) - 1
            )
            setup_histogram(
                unfolded_z_scaled_list[i_iter_choice][ibin_jetpt], get_colour(1)
            )
            leg_cunfolded_not_z.AddEntry(
                unfolded_z_scaled_list[i_iter_choice][ibin_jetpt], "unfolded", "P"
            )
            y_min_h, y_max_h = get_y_window_his(
                [unfolded_z_scaled_list[i_iter_choice][ibin_jetpt], input_data_z_scaled]
            )
            y_margin_up = 0.2
            y_margin_down = 0.05
            unfolded_z_scaled_list[i_iter_choice][ibin_jetpt].GetYaxis().SetRangeUser(
                *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
            )
            unfolded_z_scaled_list[i_iter_choice][ibin_jetpt].GetXaxis().SetRangeUser(
                round(self.lvarshape_binmin_reco[0], 2),
                round(self.lvarshape_binmax_reco[-1], 2),
            )
            ...
            draw_latex(latex)
            cunfolded_not_z.SaveAs(
                "%s/unfolded_not_%s_%s.eps"
                % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot)
            )

            # compare relative statistical uncertainties before unfolding and after

            h_unfolded_not_stat_error = TH1F(
                "h_unfolded_not_stat_error" + suffix,
                "h_unfolded_not_stat_error" + suffix,
                self.p_nbinshape_reco,
                self.varshapebinarray_reco,
            )
            for ibinshape in range(self.p_nbinshape_reco):
                error_on_unfolded = unfolded_z_scaled_list[i_iter_choice][
                    ibin_jetpt
                ].GetBinError(
                    input_mc_gen.GetXaxis().FindBin(
                        self.lvarshape_binmin_reco[ibinshape]
                    )
                )
                content_on_unfolded = unfolded_z_scaled_list[i_iter_choice][
                    ibin_jetpt
                ].GetBinContent(
                    input_mc_gen.GetXaxis().FindBin(
                        self.lvarshape_binmin_reco[ibinshape]
                ...
                if error_on_input_data != 0 and content_on_unfolded != 0:
                    h_unfolded_not_stat_error.SetBinContent(
                        ibinshape + 1,
                        (error_on_unfolded * content_on_input_data)
                        / (content_on_unfolded * error_on_input_data),
                    )
                else:
                    h_unfolded_not_stat_error.SetBinContent(ibinshape + 1, 0.0)
            cunfolded_not_stat_error = TCanvas(
                "cunfolded_not_stat_error " + suffix,
                "Ratio of stat error after to before unfolding" + suffix,
            )
            ...
            h_unfolded_not_stat_error.SetTitle(
                "Ratio of rel. stat. unc. after to before unfolding"
            )
            ...
            y_margin_up = 0.2
            y_margin_down = 0.05
            h_unfolded_not_stat_error.GetYaxis().SetRangeUser(
                *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
            )
            h_unfolded_not_stat_error.GetXaxis().SetRangeUser(
                round(self.lvarshape_binmin_reco[0], 2),
                round(self.lvarshape_binmax_reco[-1], 2),
            )
            ...
            draw_latex(latex)
            cunfolded_not_stat_error.SaveAs(
                "%s/unfolded_not_stat_error_%s.eps"
                % (self.d_resultsallpdata, suffix_plot)
            )
        ...
        leg_jetpt = TLegend(0.85, 0.45, 0.95, 0.85, "iteration")
        ...
        for i in range(self.niter_unfolding):
            ...
            if i == 0:
                unfolded_jetpt_scaled_list[i].GetXaxis().SetRangeUser(
                    round(self.lvar2_binmin_reco[0], 2),
                    round(self.lvar2_binmax_reco[-1], 2),
                )
                unfolded_jetpt_scaled_list[i].GetYaxis().SetRangeUser(
                    *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up, True)
                )
                unfolded_jetpt_scaled_list[i].SetTitle("")
                unfolded_jetpt_scaled_list[i].SetXTitle(
                    "%s (GeV/#it{c})" % self.p_latexbin2var
                )
                unfolded_jetpt_scaled_list[i].SetYTitle(
                    "1/#it{N}_{jets} d#it{N}/d%s (#it{c}/GeV)" % self.p_latexbin2var
                )
        ...
        draw_latex(latex)
        cconvergence_jetpt.SaveAs(
            "%s/convergence_%s.eps" % (self.d_resultsallpdata, self.v_var2_binning)
        )

        # jet pt convergence refolding test
        # plot the refolding test of jet pt for all iterations

        cconvergence_refolding_jetpt = TCanvas(
            "cconvergence_refolding_jetpt ", "1D output of refolding convergence"
        )
        setup_canvas(cconvergence_refolding_jetpt)
        cconvergence_refolding_jetpt.SetRightMargin(0.2)
        leg_refolding_jetpt = TLegend(0.85, 0.45, 0.95, 0.85, "iteration")
        ...
        for i in range(self.niter_unfolding):
            ...
            leg_refolding_jetpt.AddEntry(
                refolding_test_jetpt_list[i], ("%d" % (i + 1)), "P"
            )
            if i == 0:
                refolding_test_jetpt_list[i].SetTitle("")
                refolding_test_jetpt_list[i].SetXTitle(
                    "%s (GeV/#it{c})" % self.p_latexbin2var
                )
                refolding_test_jetpt_list[i].SetYTitle("refolding test")
                refolding_test_jetpt_list[i].GetYaxis().SetRangeUser(
                    *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
                )
        ...
        draw_latex(latex)
        cconvergence_refolding_jetpt.SaveAs(
            "%s/convergence_refolding_%s.eps"
            % (self.d_resultsallpdata, self.v_var2_binning)
        )
    ...
    def unfolding_closure(self):
        ...
        hzvsjetpt_reco_nocuts = unfolding_input_file.Get(
        ...
        kinematic_eff_jetpt.Divide(hjetpt_gen_nocuts)
        input_mc_gen_jetpt = input_mc_gen.ProjectionY(
            "input_mc_gen_jetpt", 1, self.p_nbinshape_gen, "e"
        )
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            input_mc_det_z.append(
                input_mc_det.ProjectionX(
                    "input_mc_det_z" + suffix, ibin2 + 1, ibin2 + 1, "e"
                )
            )
            input_mc_det_z[ibin2].Scale(
                1.0 / input_mc_det_z[ibin2].Integral(bin_int_first, -1)
            )
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            input_mc_gen_z[ibin2].Scale(
                1.0 / input_mc_gen_z[ibin2].Integral(bin_int_first, -1)
            )
            kinematic_eff.append(
                unfolding_input_file.Get("hz_gen_cuts_closure" + suffix)
            )
            hz_gen_nocuts.append(
                unfolding_input_file.Get("hz_gen_nocuts_closure" + suffix)
            )
            kinematic_eff[ibin2].Divide(hz_gen_nocuts[ibin2])
        ...
        for i in range(self.niter_unfolding):
            ...
            for ibin2 in range(self.p_nbin2_gen):
                ...
                unfolded_z = unfolded_zvsjetpt.ProjectionX(
                    "unfolded_z_%d_%s" % (i + 1, suffix), ibin2 + 1, ibin2 + 1, "e"
                )
                ...
                cclosure_z = TCanvas(
                    "cclosure_z_%d_%s" % (i + 1, suffix),
                    "1D output of closure" + suffix,
                )
                ...
                unfolded_z.GetXaxis().SetRangeUser(
                    round(self.lvarshape_binmin_reco[0], 2),
                    round(self.lvarshape_binmax_reco[-1], 2),
                )
                unfolded_z.SetTitle("")
            ...
            cclosure_jetpt = TCanvas(
                "cclosure_jetpt_%d" % (i + 1), "1D output of closure"
            )
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            cconvergence_closure_z = TCanvas(
                "cconvergence_closure_z " + suffix,
                "1D output of closure convergence" + suffix,
            )
            setup_canvas(cconvergence_closure_z)
            cconvergence_closure_z.SetRightMargin(0.2)
            leg_closure = TLegend(0.85, 0.45, 0.95, 0.85, "iteration")
            setup_legend(leg_closure)
            l_his = [
                unfolded_z_closure_list[i][ibin2] for i in range(self.niter_unfolding)
            ]
            y_min_h, y_max_h = get_y_window_his(l_his)
            y_margin_up = 0.15
            y_margin_down = 0.05
            for i in range(self.niter_unfolding):
                ...
                leg_closure.AddEntry(
                    unfolded_z_closure_list[i][ibin2], ("%d" % (i + 1)), "P"
                )
                if i == 0:
                    unfolded_z_closure_list[i][ibin2].GetXaxis().SetRangeUser(
                        round(self.lvarshape_binmin_gen[0], 2),
                        round(self.lvarshape_binmax_gen[-1], 2),
                    )
                    unfolded_z_closure_list[i][ibin2].GetYaxis().SetRangeUser(
                        *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
                    )
            ...
            draw_latex(latex)
            cconvergence_closure_z.SaveAs(
                "%s/convergence_closure_%s_%s.eps"
                % (self.d_resultsallpdata, self.v_varshape_binning, suffix)
            )

        # plot closure test for jet pt for all iterations together

        cconvergence_closure_jetpt = TCanvas(
            "cconvergence_closure_jetpt ", "1D output of closure convergence"
        )
        setup_canvas(cconvergence_closure_jetpt)
        cconvergence_closure_jetpt.SetRightMargin(0.2)
        leg_closure_jetpt = TLegend(0.85, 0.45, 0.95, 0.85, "iteration")
        ...
        for i in range(self.niter_unfolding):
            ...
            leg_closure_jetpt.AddEntry(
                unfolded_jetpt_closure_list[i], ("%d" % (i + 1)), "P"
            )
            if i == 0:
                unfolded_jetpt_closure_list[i].GetXaxis().SetRangeUser(
                    round(self.lvar2_binmin_gen[0], 2),
                    round(self.lvar2_binmax_gen[-1], 2),
                )
                unfolded_jetpt_closure_list[i].GetYaxis().SetRangeUser(
                    *get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
                )
                unfolded_jetpt_closure_list[i].SetTitle("")
                unfolded_jetpt_closure_list[i].SetXTitle(
                    "%s (GeV/#it{c})" % self.p_latexbin2var
                )
        ...
        draw_latex(latex)
        cconvergence_closure_jetpt.SaveAs(
            "%s/convergence_closure_%s.eps"
            % (self.d_resultsallpdata, self.v_var2_binning)
        )
    ...
    def jetsystematics(self):
        ...
        for i_powheg in range(len(self.powheg_prompt_variations)):
            path = "%s%s.root" % (
                ...
                self.powheg_prompt_variations[i_powheg],
            )
            input_powheg_sys_i = self.get_simulated_yields(path, 2, True)
            if not input_powheg_sys_i:
                self.logger.fatal(make_message_notfound("simulated yields", path))
            input_powheg_sys_i.SetName(
                "fh2_prompt_%s_%d" % (self.v_varshape_binning, i_powheg)
            )
            input_powheg_sys.append(input_powheg_sys_i)
            input_powheg_xsection_sys_i = input_powheg_sys_i.Clone(
                input_powheg_sys_i.GetName() + "_xsec"
            )
            input_powheg_xsection_sys.append(input_powheg_xsection_sys_i)
        input_powheg_z = []
        input_powheg_xsection_z = []
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            input_powheg_z[ibin2].Scale(
                1.0
                / input_powheg_z[ibin2].Integral(
                    input_powheg_z[ibin2].FindBin(self.lvarshape_binmin_reco[0]),
                    input_powheg_z[ibin2].FindBin(self.lvarshape_binmin_reco[-1]),
                ),
                "width",
            )
            ...
            input_powheg_xsection_sys_z_iter = []
            for i_powheg in range(len(self.powheg_prompt_variations)):
                input_powheg_sys_z_iter.append(
                    input_powheg_sys[i_powheg].ProjectionX(
                        "input_powheg_sys_z"
                        + self.powheg_prompt_variations[i_powheg]
                        + suffix,
                        ibin2 + 1,
                ...
                input_powheg_sys_z_iter[i_powheg].Scale(
                    1.0
                    / input_powheg_sys_z_iter[i_powheg].Integral(
                        input_powheg_sys_z_iter[i_powheg].FindBin(
                            self.lvarshape_binmin_reco[0]
                        ),
                        input_powheg_sys_z_iter[i_powheg].FindBin(
                            self.lvarshape_binmin_reco[-1]
                        ),
                    ),
                    "width",
                )
                input_powheg_xsection_sys_z_iter.append(
                    input_powheg_xsection_sys[i_powheg].ProjectionX(
                        "input_powheg_xsection_sys_z"
                        + self.powheg_prompt_variations[i_powheg]
                        + suffix,
                        ibin2 + 1,
            ...
            input_powheg_xsection_sys_z.append(input_powheg_xsection_sys_z_iter)
            tg_powheg.append(tg_sys(input_powheg_z[ibin2], input_powheg_sys_z[ibin2]))
            tg_powheg_xsection.append(
                tg_sys(
                    input_powheg_xsection_z[ibin2], input_powheg_xsection_sys_z[ibin2]
                )
            )
        ...
        for sys_cat in range(self.n_sys_cat):
            ...
            for sys_var, varname in enumerate(self.systematic_varnames[sys_cat]):
                path = path_def.replace(
                    string_default, self.systematic_catnames[sys_cat] + "/" + varname
                )
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            for sys_cat in range(self.n_sys_cat):
                ...
                for sys_var in range(self.systematic_variations[sys_cat]):
                    ...
                    input_histograms_syscatvar.append(
                        input_files_sys[sys_cat][sys_var].Get(name_his)
                    )
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            l_his_all = [
                his_var for l_cat in input_histograms_sys[ibin2] for his_var in l_cat
            ] + [input_histograms_default[ibin2]]
            ...
            input_histograms_default[ibin2].SetYTitle(
                "1/#it{N}_{jets} d#it{N}/d%s" % self.v_varshape_latex
            )
            ...
            for sys_cat in range(self.n_sys_cat):
                ...
                nsys = 0
                csysvar_each = TCanvas(
                    "csysvar_%s_%s" % (suffix2, suffix),
                    "systematic variations" + suffix2 + suffix,
                )
                setup_canvas(csysvar_each)
                csysvar_each.SetRightMargin(0.25)
                leg_sysvar_each = TLegend(
                    0.77, 0.2, 0.95, 0.85, self.systematic_catlabels[sys_cat]
                )  # Rg
                setup_legend(leg_sysvar_each)
                leg_sysvar_each.AddEntry(
                    input_histograms_default[ibin2], "default", "P"
                )
                setup_histogram(input_histograms_default[ibin2])
                y_min, y_max = get_y_window_his(
                    input_histograms_sys[ibin2][sys_cat]
                    + [input_histograms_default[ibin2]]
                )
                y_margin_up = 0.15
                y_margin_down = 0.05
                ...
                csysvar_each.SaveAs(
                    "%s/sys_var_%s_%s.eps" % (self.d_resultsallpdata, suffix2, suffix)
                )
            ...
            for ibinshape in range(self.p_nbinshape_gen):
                ...
                for sys_cat in range(self.n_sys_cat):
                    ...
                    for sys_var in range(self.systematic_variations[sys_cat]):
                        ...
                        error = input_histograms_sys[ibin2][sys_cat][
                            sys_var
                        ].GetBinContent(
                            ibinshape + bin_first
                        ) - input_histograms_default[
        ...
        tgsys = (
            []
        )  # list of graphs with combined absolute uncertainties for all pt_jet bins
        tgsys_cat = (
            []
        )  # list of graphs with relative uncertainties for all categories, pt_jet bins
        for ibin2 in range(self.p_nbin2_gen):
            ...
            for ibinshape in range(self.p_nbinshape_gen):
                shapebins_centres.append(
                    input_histograms_default[ibin2].GetBinCenter(ibinshape + 1)
                )
                shapebins_contents.append(
                    input_histograms_default[ibin2].GetBinContent(ibinshape + 1)
                )
                shapebins_widths_up.append(
                    input_histograms_default[ibin2].GetBinWidth(ibinshape + 1) * 0.5
                )
                shapebins_widths_down.append(
                    input_histograms_default[ibin2].GetBinWidth(ibinshape + 1) * 0.5
                )
            ...
            tgsys_cat_z = (
                []
            )  # list of graphs with relative uncertainties for all categories in a given pt_jet bin
            for sys_cat in range(self.n_sys_cat):
                ...
                for ibinshape in range(self.p_nbinshape_gen):
                    ...
                    shapebins_error_up_cat.append(
                        sys_up[ibin2][ibinshape][sys_cat]
                        / input_histograms_default[ibin2].GetBinContent(ibinshape + 1)
                    )
        ...
        file_sys_out = TFile.Open(
            "%s/systematics_results.root" % self.d_resultsallpdata, "recreate"
        )
        for ibin2 in range(self.p_nbin2_gen):
            suffix = "%s_%.2f_%.2f" % (
                self.v_var2_binning,
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            h_default_stat_err.append(
                input_histograms_default[ibin2].Clone("h_default_stat_err" + suffix)
            )
        ...
        file_sim_out = TFile.Open(
            "%s/simulations.root" % self.d_resultsallpdata, "recreate"
        )
        input_pythia8 = []
        ...
        for i_pythia8 in range(len(self.pythia8_prompt_variations)):
            path = "%s%s.root" % (
                ...
                self.pythia8_prompt_variations[i_pythia8],
            )
            input_pythia8_i = self.get_simulated_yields(path, 2, True)
            if not input_pythia8_i:
                self.logger.fatal(make_message_notfound("simulated yields", path))
            input_pythia8_i.SetName(
                "fh2_pythia_prompt_%s_%d" % (self.v_varshape_binning, i_pythia8)
            )
            input_pythia8.append(input_pythia8_i)
            input_pythia8_xsection_i = input_pythia8_i.Clone(
                input_pythia8_i.GetName() + "_xsec"
            )
            input_pythia8_xsection.append(input_pythia8_xsection_i)

            # Ensure correct binning: x - shape, y - jet pt
            if not equal_binning_lists(
                input_pythia8[i_pythia8], list_x=self.varshaperanges_gen
            ):
                self.logger.fatal("Error: Incorrect binning in x.")
            if not equal_binning_lists(
                input_pythia8[i_pythia8], list_y=self.var2ranges_gen
            ):
                self.logger.fatal("Error: Incorrect binning in y.")
            if not equal_binning_lists(
                input_pythia8_xsection[i_pythia8], list_x=self.varshaperanges_gen
            ):
                self.logger.fatal("Error: Incorrect binning in x.")
            if not equal_binning_lists(
                input_pythia8_xsection[i_pythia8], list_y=self.var2ranges_gen
            ):
                self.logger.fatal("Error: Incorrect binning in y.")
            ...
            for ibin2 in range(self.p_nbin2_gen):
                ...
                input_pythia8_z_jetpt.append(
                    input_pythia8[i_pythia8].ProjectionX(
                        "input_pythia8"
                        + self.pythia8_prompt_variations[i_pythia8]
                        + suffix,
                        ibin2 + 1,
                ...
                input_pythia8_z_jetpt[ibin2].Scale(
                    1.0 / input_pythia8_z_jetpt[ibin2].Integral(), "width"
                )
                pythia8_out = input_pythia8_z_jetpt[ibin2]
                file_sim_out.cd()
                pythia8_out.Write()
                pythia8_out.SetDirectory(0)
                input_pythia8_xsection_z_jetpt.append(
                    input_pythia8_xsection[i_pythia8].ProjectionX(
                        "input_pythia8_xsection"
                        + self.pythia8_prompt_variations[i_pythia8]
                        + suffix,
                        ibin2 + 1,
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            cfinalwsys = TCanvas(
                "cfinalwsys " + suffix,
                "final result with systematic uncertainties" + suffix,
            )
            setup_canvas(cfinalwsys)
            leg_finalwsys = TLegend(0.7, 0.75, 0.85, 0.85)
            ...
            latex1 = TLatex(
                0.15,
                0.77,
                "%s in charged jets, anti-#it{k}_{T}, #it{R} = 0.4"
                % self.p_latexnhadron,
            )
            draw_latex(latex1)
            latex2 = TLatex(
            ...
            latex_SD = TLatex(
                0.15, 0.62, "Soft Drop (#it{z}_{cut} = 0.1, #it{#beta} = 0)"
            )
            draw_latex(latex_SD)
            cfinalwsys.SaveAs(
                "%s/%s_final_wsys_%s.pdf" % (self.d_resultsallpdata, self.shape, suffix)
            )

            # plot the results with systematic uncertainties and models

            leg_pos = [0.55, 0.65, 0.8, 0.85]

            cfinalwsys_wmodels = TCanvas(
                "cfinalwsys_wmodels " + suffix,
                "final result with systematic uncertainties with models" + suffix,
            )
            ...
            y_min_h, y_max_h = get_y_window_his(
                [input_histograms_default[ibin2], input_powheg_z[ibin2]]
                + [
                    input_pythia8_z[i][ibin2]
                    for i in range(len(self.pythia8_prompt_variations))
                ]
            )
            y_min = min(y_min_g, y_min_h)
            y_max = max(y_max_g, y_max_h)
            y_margin_up = 0.4
            y_margin_down = 0.05
            y_plot_min, y_plot_max = get_plot_range(
                y_min, y_max, y_margin_down, y_margin_up
            )
            input_histograms_default[ibin2].GetYaxis().SetRangeUser(
            ...
            leg_finalwsys_wmodels.AddEntry(
                input_powheg_z[ibin2], "POWHEG #plus PYTHIA 6", "P"
            )
            input_powheg_z[ibin2].Draw("same")
            ...
            for i_pythia8 in range(len(self.pythia8_prompt_variations)):
                setup_histogram(
                    input_pythia8_z[i_pythia8][ibin2],
                    get_colour(i_pythia8 + 2),
                    get_marker(i_pythia8 + 2),
                    2.0,
                )
                leg_finalwsys_wmodels.AddEntry(
                    input_pythia8_z[i_pythia8][ibin2],
                    self.pythia8_prompt_variations_legend[i_pythia8],
                    "P",
                )
            ...
            cfinalwsys_wmodels.SaveAs(
                "%s/%s_final_wsys_wmodels_%s.pdf"
                % (self.d_resultsallpdata, self.shape, suffix)
            )
            ...
            cfinalwsys_wmodels_new, _ = make_plot(
                "cfinalwsys_wmodels_new_" + suffix,
                list_obj=list_obj,
                labels_obj=labels_obj,
                opt_leg_g="FP",
                opt_plot_g="2",
                ...
                leg_pos=leg_pos,
                margins_y=[0.05, 0.4],
                title=";%s;1/#it{N}_{jets} d#it{N}/d%s"
                % (self.v_varshape_latex, self.v_varshape_latex),
            )
            ...
            for text_latex in list_text:
                ...
                y_latex -= self.y_step
            cfinalwsys_wmodels_new.SaveAs(
                "%s/%s_final_wsys_wmodels_%s_new.pdf"
                % (self.d_resultsallpdata, self.shape, suffix)
            )
            ...
            # preliminary figure
            crelativesys = TCanvas(
                "crelativesys " + suffix, "relative systematic uncertainties" + suffix
            )
            gStyle.SetErrorX(0)
            ...
            leg_relativesys = TLegend(0.68, 0.6, 0.88, 0.91)
            ...
            for sys_cat in range(self.n_sys_cat):
                ...
                tgsys_cat[ibin2][sys_cat].GetYaxis().SetTitle(
                    "relative systematic uncertainty"
                )
                tgsys_cat[ibin2][sys_cat].GetXaxis().SetTitleOffset(
                    self.offsets_axes[0]
                )
                tgsys_cat[ibin2][sys_cat].GetYaxis().SetTitleOffset(
                    self.offsets_axes[1]
                )
                leg_relativesys.AddEntry(
                    tgsys_cat[ibin2][sys_cat], self.systematic_catlabels[sys_cat], "F"
                )
                if sys_cat == 0:
        ...
        h_feeddown_fraction = (
            []
        )  # list of the central feed-down fractions for all pt_jet bins
        h_feeddown_fraction_variations = (
            []
        )  # list of feed-down fractions for all POWHEG variations and all pt_jet bins
        tg_feeddown_fraction = (
            []
        )  # list of graphs with the spread of values for all pt_jet bins
        for ibin2 in range(self.p_nbin2_reco):
            suffix = "%s_%.2f_%.2f" % (
                self.v_var2_binning,
                self.lvar2_binmin_reco[ibin2],
            ...
            h_feeddown_fraction_variations_niter = (
                []
            )  # list of feed-down fractions for all POWHEG variations in a given pt_jet bin
            ...
            for i_powheg in range(len(self.powheg_nonprompt_varnames)):
                h_feeddown_fraction_variations_niter.append(
                    file_feeddown_variations[i_powheg].Get("feeddown_fraction" + suffix)
                )
            ...
            tg_feeddown_fraction.append(
                tg_sys(
                    h_feeddown_fraction[ibin2], h_feeddown_fraction_variations[ibin2]
                )
            )

            cfeeddown_fraction = TCanvas(
                "cfeeddown_fraction " + suffix, "feeddown fraction" + suffix
            )
            setup_canvas(cfeeddown_fraction)
            cfeeddown_fraction.SetLeftMargin(0.13)
            leg_fd = TLegend(0.67, 0.6, 0.85, 0.85)
            ...
            y_margin_up = 0.45
            y_margin_down = 0.05
            h_feeddown_fraction[ibin2].GetYaxis().SetRangeUser(
                *get_plot_range(y_min, y_max, y_margin_down, y_margin_up)
            )
            h_feeddown_fraction[ibin2].GetXaxis().SetRangeUser(
                round(self.lvarshape_binmin_reco[0], 2),
                round(self.lvarshape_binmax_reco[-1], 2),
            )
            ...
            latex1 = TLatex(
                0.18,
                0.77,
                "%s in charged jets, anti-#it{k}_{T}, #it{R} = 0.4, #left|#it{#eta}_{jet}#right| #leq 0.5"
                % self.p_latexnhadron,
            )
            draw_latex(latex1)
            latex2 = TLatex(
            ...
            # draw_latex(latex7)
            cfeeddown_fraction.SaveAs(
                "%s/%s_feeddown_fraction_var_%s.eps"
                % (self.d_resultsallpdata, self.shape, suffix_plot)
            )
            cfeeddown_fraction.SaveAs(
                "%s/%s_feeddown_fraction_var_%s.pdf"
                % (self.d_resultsallpdata, self.shape, suffix_plot)
            )
            ...
            if ibin2 == 1:
                ...
                y_margin_up = 0.5
                y_margin_down = 0.05
                c_fd_fr_sys, list_obj_data_new = make_plot(
                    "c_fd_fr_sys_" + suffix,
                    size=self.size_can,
                    list_obj=list_obj,
                    labels_obj=labels_obj,
    ...
    def get_simulated_yields(self, file_path: str, dim: int, prompt: bool):
        ...
        print(
            "Starting the histogram extraction from an MC tree\nInput file: %s"
            % file_path
        )
        ...
        # cut on jet pt
        df_sim = seldf_singlevar(
            df_sim,
            self.v_var2_binning,
            self.lvar2_binmin_gen[0],
            self.lvar2_binmax_gen[-1],
        )
        # cut on hadron pt
        df_sim = seldf_singlevar(
            df_sim, self.v_var_binning, self.lpt_finbinmin[0], self.lpt_finbinmax[-1]
        )
        # cut on shape
        df_sim = seldf_singlevar(
            df_sim,

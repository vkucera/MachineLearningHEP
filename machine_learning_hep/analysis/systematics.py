...
class SystematicsMLWP:  # pylint: disable=too-few-public-methods, too-many-instance-attributes
    ...
    def __init__(
        ...
        self.min_signif_fit = datap["systematics"]["probvariation"].get(
            "min_signif_fit", -1.0
        )
        self.max_red_chi2_fit = datap["systematics"]["probvariation"].get(
            "max_red_chi2_fit", -1.0
        )
        ...
        if self.mcopt is not None:
            ...
            self.p_cutvar_minrange = list(
                map(itemgetter(self.mcopt), self.p_cutvar_minrange)
            )
            self.p_cutvar_maxrange = list(
                map(itemgetter(self.mcopt), self.p_cutvar_maxrange)
            )
    ...
    def __read_nominal_fit_values(self):
        ...
        self.nominal_means = [
            [None] * ana_n_first_binning for _ in range(ana_n_second_binning)
        ]
        self.nominal_sigmas = [
            [None] * ana_n_first_binning for _ in range(ana_n_second_binning)
        ]
    ...
    def __define_cutvariation_limits(self):  # pylint: disable=too-many-statements
        ...
        results_dirs_periods = [
            join(d, "tmp_ml_wp_limits")
            for d in datap["analysis"][self.typean]["mc"]["results"]
        ]
        results_dir_all = join(
            datap["analysis"][self.typean]["mc"]["resultsallp"], "tmp_ml_wp_limits"
        )
        ...
        analyzer_effs = self.nominal_analyzer_merged.__class__(
            datap, self.case, self.typean, None
        )
        ...
        for ibin1 in range(ana_n_first_binning):
            nominal_effs[ibin1], _ = self.nominal_analyzer_merged.get_efficiency(
                ibin1, 0
            )
        ...
        def compute_new_boundaries(wps, boundaries):
            ...
            if found_all_boundaries(boundaries):
                return
            wps_strings = [
                "y_test_prob%s>%s" % (modelname, wps[ipt]) for ipt in range(n_pt_bins)
            ]
            if self.mcopt is not None:
                ...
                if self.mcopt == 0:
                    wps_strings = [
                        "%s<=%s and %s>=%s"
                        % (probvar0, wps[ipt], probvar1, self.cent_cv_cut_orig[ipt][1])
                        for ipt in range(n_pt_bins)
                    ]
                    wps_multi = [
                        [wps[ipt], self.cent_cv_cut_orig[ipt][1]]
                        for ipt in range(n_pt_bins)
                    ]
                elif self.mcopt == 1:
                    wps_strings = [
                        "%s<=%s and %s>=%s"
                        % (probvar0, self.cent_cv_cut_orig[ipt][0], probvar1, wps[ipt])
                        for ipt in range(n_pt_bins)
                    ]
                    wps_multi = [
                        [self.cent_cv_cut_orig[ipt][0], wps[ipt]]
                        for ipt in range(n_pt_bins)
                    ]
                else:
            ...
            for ibin1 in range(ana_n_first_binning):
                ...
                if (
                    abs(eff_new - nominal_effs[ibin1]) / nominal_effs[ibin1]
                    < self.p_maxperccutvar
        ...
        for icv in range(ncutvar_temp):
            ...
            wps = [
                self.p_cutvar_minrange[ipt] + icv * stepsmin[ipt]
                for ipt in range(n_pt_bins)
            ]
            compute_new_boundaries(wps, self.min_cv_cut)
            wps = [
                self.p_cutvar_maxrange[ipt] - icv * stepsmax[ipt]
                for ipt in range(n_pt_bins)
            ]
            compute_new_boundaries(wps, self.max_cv_cut)

        print(
            "Limits for cut variation defined, based on eff %-var of: ",
            self.p_maxperccutvar,
        )
        ...
        for ipt in range(n_pt_bins):
            ...
            for icv in range(self.p_ncutvar):
                ...
                if self.mcopt == 0:
                    ...
                    self.ml_wps[self.p_ncutvar + icv].append(
                        [upper_cut, self.cent_cv_cut_orig[ipt][1]]
                    )
                elif self.mcopt == 1:
                    self.ml_wps[icv].append([self.cent_cv_cut_orig[ipt][0], lower_cut])
                    self.ml_wps[self.p_ncutvar + icv].append(
                        [self.cent_cv_cut_orig[ipt][0], upper_cut]
                    )
                else:
    ...
    def __prepare_trial(self, i_trial):
        ...
        datap["analysis"][self.typean]["SetFixGaussianSigma"] = [True] * len(
            self.nominal_sigmas[0]
        )
        datap["analysis"][self.typean]["SetInitialGaussianSigma"] = [True] * len(
            self.nominal_sigmas[0]
        )
        datap["analysis"][self.typean]["SetInitialGaussianMean"] = [True] * len(
            self.nominal_sigmas[0]
        )
        ...
        self.analyzers_syst[i_trial] = self.nominal_analyzer_merged.__class__(
            datap, self.case, self.typean, None
        )
    ...
    def __make_summary_plot(self, name, ibin2, successful):
        ...
        for ipt in range(nptbins):
            ...
            gr[ipt].SetPoint(
                0, self.cent_cv_cut[ipt], nominal_histo.GetBinContent(ipt + 1)
            )
            ...
            for iml, succ in enumerate(successful_tmp):
                ...
                gr[ipt].SetPointError(
                    iml + 1, 0.0001, histos[succ].GetBinError(ipt + 1)
                )
    ...
    def ml_systematics(self, do_only_analysis=False, resume=False):
        ...
        save_path = join(
            self.nominal_analyzer_merged.d_resultsallpdata, self.syst_out_dir
        )
        if not exists(save_path):
            makedirs(save_path)

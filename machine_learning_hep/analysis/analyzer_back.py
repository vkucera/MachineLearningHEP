...
class Analyzer:
    ...
    def __init__(self, datap, case, typean, resultsdata, resultsmc, valdata, valmc):
    ...
    def fit(self):
        ...
        self.fitter = MLFitter(
            self.datap, self.typean, self.n_filemass, self.n_filemass_mc
        )
        ...
        fileout.Close()
        fileout_name = os.path.join(
            self.d_resultsallpdata, f"{self.fits_dirname}_{self.case}_{self.typean}"
        )
    ...
    def plot_multi_trial(self):
        ...
        if not os.path.exists(self.d_mt_results_path):
            self.logger.error(
                "Could not find multi trial results directory %s. Skip...",
        ...
        mt_derived_filename = self.make_file_path(
            self.d_mt_results_path,
            "multi_trial_summary",
            "root",
            None,
        ...
        max_chisquare_ndf = self.mt_syst_dict.get("max_chisquare_ndf", 2.0)
        ...
        consider_free_sigma = self.mt_syst_dict.get(
            "consider_free_sigma", [False for i in range(self.p_nptbins)]
        )

        for imult in range(self.p_nbin2):
            h_nominal = TH1F(
                "h_nominal_sum", "", self.p_nptbins, array("d", self.ptranges)
            )
            h_fit_all = TH1F(
                "h_yield_sum", "", self.p_nptbins, array("d", self.ptranges)
            )
            h_fit_all = TH1F(
                "h_yield_sum", "", self.p_nptbins, array("d", self.ptranges)
            )
            h_bincount_all0 = TH1F(
                "h_bincount_sum0", "", self.p_nptbins, array("d", self.ptranges)
            )
            h_bincount_all1 = TH1F(
                "h_bincount_sum1", "", self.p_nptbins, array("d", self.ptranges)
            )
            ...
            for ipt in range(self.p_nptbins):
                ...
                mt_filename = self.make_file_path(
                    self.d_mt_results_path, "multi_trial", "root", None, suffix_write
                )
                if not os.path.exists(mt_filename):
                    self.logger.warning(
                        "Multi trial file %s does not exist. MT might have "
                        "failed before. Skipping...",
                        mt_filename,
                    )
                    continue
                # Get the nominal fit
                ...
                title = (
                    f"{self.lpt_finbinmin[ipt]} GeV/c < {self.v_var_binning} < "
            ...
            filename_mt_summary = self.make_file_path(
                self.d_mt_results_path, "multi_trial_summary", "eps", None, [imult]
            )

            plot_histograms(
                [h_nominal, h_fit_all, h_bincount_all0, h_bincount_all1],
                False,
                True,
                [
                    "central fit",
                ...
                "MT / central",
                filename_mt_summary,
                colors=[kBlack, kBlue, kGreen + 2, kOrange + 5],
            )
            column_names = [
                "central fit",
                "mean MT fit",
            ...
            row_names = [
                f"{self.lpt_finbinmin[ipt]} GeV/c < {self.v_var_binning} < "
                f"{self.lpt_finbinmax[ipt]} GeV/c"
                for ipt in range(self.p_nptbins)
            ]
            rows = []
            for b in range(h_nominal.GetNbinsX()):
                ...
                rel_centr_fit = (
                    yield_nominal_err / yield_nominal if yield_nominal > 0.0 else 0.0
                )
                rel_mt_fit = (
                    (yield_nominal - yield_mt_fit) / yield_nominal
                    if yield_nominal > 0.0
                    else 0.0
                )
                rel_mt_bc0 = (
                    (yield_nominal - yield_mt_bc0) / yield_nominal
                    if yield_nominal > 0.0
                    else 0.0
                )
                rel_mt_bc1 = (
                    (yield_nominal - yield_mt_bc1) / yield_nominal
                    if yield_nominal > 0.0
                    else 0.0
                )
                rows.append(
                    [
                        f"{yield_nominal:.2f} ({yield_nominal_err:.2f})",
            ...
            filename_mt_summary = self.make_file_path(
                self.d_mt_results_path, "multi_trial_summary", "tex", None, [imult]
            )
            make_latex_table(
                column_names, row_names, rows, caption, filename_mt_summary
            )
    ...
    def feeddown(self):
        ...
        file_out = TFile.Open(
            "%s/feeddown%s%s.root" % (self.d_resultsallpmc, self.case, self.typean),
            "recreate",
        )
        ...
        bins_final = np.array(
            [
                his_resp_fd.GetYaxis().GetBinLowEdge(i)
                for i in range(1, his_resp_fd.GetYaxis().GetNbins() + 2)
            ]
        )
        ...
        can_resp_fd.SaveAs(
            "%s/ResponseFD%s%s.eps" % (self.d_resultsallpmc, self.case, self.typean)
        )
        ...
        his_sim_fd.Scale(
            1.0 / n_jets_gen
        )  # Normalise by the total number of selected jets.
        arr_sim_fd = hist2array(his_sim_fd).T
        can_sim_fd = TCanvas(
            "can_sim_fd", "Simulated pt cand vs. pt jet of non-prompt jets", 800, 800
        )
        ...
        can_sim_fd.SaveAs(
            "%s/GenFD%s%s.eps" % (self.d_resultsallpmc, self.case, self.typean)
        )
        ...
        for imult in range(self.p_nbin2):
            ...
            his_eff_pr.GetYaxis().SetTitle(
                "reconstruction efficiency %s %s" % (self.p_latexnhadron, self.typean)
            )
            his_eff_pr.GetYaxis().SetRangeUser(0, 0.6)
            leg_eff = TLegend(0.5, 0.15, 0.7, 0.35)
            ...
            leg_eff.Draw()
            can_eff.SaveAs(
                "%s/Efficiency%s%s%s.eps"
                % (self.d_resultsallpmc, self.case, self.typean, imult)
            )
        ...
        for i_ptjet in range(self.p_nbin2):
            ...
            for i_z in range(his_sim_z_fd_2d_eff.GetNbinsX()):
                ...
                his_sim_z_fd_2d_eff.SetBinContent(
                    bin_z, bin_ptjet, his_sim_ptc_fd.Integral()
                )
        ...
        can_resp_z.SetLogz()
        can_resp_z.SaveAs(
            "%s/Feeddown-z-response_%s%s.eps"
            % (self.d_resultsallpmc, self.case, self.typean)
        )

        for i_ptjet in range(self.p_nbin2):
            bin_ptjet = i_ptjet + 1
            can_ff_fd_fold = TCanvas(
                "can_fd_z_all%d" % i_ptjet, "Feeddown FF all", 800, 600
            )
            his_ff_fd_gen = his_sim_z_fd_2d_gen.ProjectionX(
                "ff_gen%d" % i_ptjet, bin_ptjet, bin_ptjet
            )
            his_ff_fd_eff = his_sim_z_fd_2d_eff.ProjectionX(
                "ff_eff%d" % i_ptjet, bin_ptjet, bin_ptjet
            )
            his_ff_fd_fold = his_sim_z_fd_2d_folded.ProjectionX(
                "ff_fold%d" % i_ptjet, bin_ptjet, bin_ptjet
            )
            his_ff_fd_gen.GetYaxis().SetTitle(
                "#frac{1}{#it{N}_{jet}} #frac{d#it{N}}{d#it{z}}"
            )
            his_ff_fd_gen.GetYaxis().SetTitleOffset(1.6)
            his_ff_fd_gen.GetYaxis().SetTitleFont(42)
            his_ff_fd_gen.GetYaxis().SetLabelFont(42)
            his_ff_fd_gen.GetYaxis().SetRangeUser(
                0.0,
                1.1
                * max(
                    his_ff_fd_gen.GetMaximum(),
                    his_ff_fd_eff.GetMaximum(),
                    his_ff_fd_fold.GetMaximum(),
                ),
            )
            his_ff_fd_gen.SetLineColor(1)
            his_ff_fd_eff.SetLineColor(2)
            his_ff_fd_fold.SetLineColor(3)
            leg_ff_fd = TLegend(0.15, 0.65, 0.35, 0.85)
            ...
            can_ff_fd_fold.SaveAs(
                "%s/Feeddown-z-all_%s%s%s.eps"
    ...
    def side_band_sub(self):
        ...
        for imult in range(self.p_nbin2):
            ...
            for ipt in range(self.p_nptbins):
                ...
                csubsig.SaveAs(
                    "%s/side_band_sub_signal%s%s_%s.eps"
                    % (self.d_resultsallpdata, self.case, self.typean, suffix)
                )

                csubbkg = TCanvas(
                    "csubbkg" + suffix, "The Side-Band Sub Background Canvas"
                )
                ...
                csubbkg.SaveAs(
                    "%s/side_band_sub_background%s%s_%s.eps"
                    % (self.d_resultsallpdata, self.case, self.typean, suffix)
                )
                ...
                csubz.SaveAs(
                    "%s/side_band_sub%s%s_%s.eps"
                    % (self.d_resultsallpdata, self.case, self.typean, suffix)
                )

                legsigbkgsubz = TLegend(0.2, 0.65, 0.35, 0.85)
                ...
                hzsig.GetYaxis().SetRangeUser(
                    0.0,
                    max(
                        hzsig.GetBinContent(hzsig.GetMaximumBin()),
                        hzbkg_scaled.GetBinContent(hzbkg_scaled.GetMaximumBin()),
                        hzsub_noteffscaled.GetBinContent(
                            hzsub_noteffscaled.GetMaximumBin()
                        ),
                    )
                    * 1.2,
                )
    ...
    def studyevents(self):
        ...
        for ivar, var in enumerate(varlist):
            ...
            for trigger in triggerlist:
                ...
                if ivar == 0:
                    ...
                    func.SetRange(550.0, 1100.0)
                    func.SetLineWidth(1)
                    hratio.Fit(func, "L", "", 550, 1100)
                    func.Draw("same")
                    func.SetLineColor(ivar + 1)
                if ivar == 1:
                    func.SetParameters(100, 0.1, 50)
                    func.SetParLimits(1, 0.0, 10.0)
                    func.SetParLimits(2, 0.0, 200.0)
                    func.SetRange(45.0, 105)
                    func.SetLineWidth(1)
                    hratio.Fit(func, "L", "", 45, 105)
                    func.SetLineColor(ivar + 1)
                if ivar == 2:
                    func.SetParameters(315, -30.0, 0.2)
                    func.SetParLimits(1, -100.0, 0.0)
                    func.SetParLimits(2, 0.0, 0.5)
                    func.SetRange(0.0, 0.15)
                    func.SetLineWidth(1)
                    hratio.Fit(func, "w", "", 0, 0.15)
                    func.SetLineColor(ivar + 1)
                func.Write()
                funcnorm = func.Clone("funcnorm_%s_%s" % (trigger, var))
                funcnorm.FixParameter(
                    0, funcnorm.GetParameter(0) / funcnorm.GetMaximum()
                )
                funcnorm.Write()
                ctrigger.cd(3)
                maxhistx = 0
                ...
                ctrigger.cd(4)
                gPad.SetLogy()
                leg1 = TLegend(0.2, 0.75, 0.4, 0.85)
                ...
                for ibin in range(hmult.GetNbinsX()):
                    ...
                    hmultweighted.SetBinContent(
                        ibin + 1, hmult.GetBinContent(ibin + 1) / myweight
                    )
                hmult.Write()
                hmultweighted.Write()
                ...
                ctrigger.cd(5)
                leg2 = TLegend(0.2, 0.75, 0.4, 0.85)
                ...
                linear = TF1(
                    "lin_%s_%s" % (trigger, var), "[0]", fulleffmin, fulleffmax
                )
                ...
                hratioMBuncorr.Scale(1.0 / linear.GetParameter(0))
                hratioMBcorr.Scale(1.0 / linear.GetParameter(0))
                ...
                leg2.Draw()
                ctrigger.cd(6)
                ptext = TPaveText(0.05, 0.1, 0.95, 0.8)
                ptext.AddText("%s" % (trigger))
                ptext.AddText("MB events=%f M" % (float(hden.Integral()) / 1.0e6))
                ptext.AddText(
                    "%s events=%f M" % (trigger, float(hmult.Integral()) / 1.0e6)
                )
                ptext.Draw()
                ...
                for ibin in range(hmult.GetNbinsX()):
                    ...
                    hselweighted.SetBinContent(
                        ibin + 1, hsel.GetBinContent(ibin + 1) / myweight
                    )
                    hnovtxweighted.SetBinContent(
                        ibin + 1, hnovtx.GetBinContent(ibin + 1) / myweight
                    )
                    hvtxoutweighted.SetBinContent(
                        ibin + 1, hvtxout.GetBinContent(ibin + 1) / myweight
                    )
                hsel.Write()
                hnovtx.Write()
                hvtxout.Write()
        ...
        if hntrklsperc:
            ...
            gPad.SetLogx()
            gPad.SetLogz()
            hntrklsperc.GetYaxis().SetRangeUser(0.0, 200.0)
        ...
        if hntrklsv0m:
            cscatter.cd(3)
            hntrklsv0m.GetYaxis().SetRangeUser(0.0, 200.0)
            gPad.SetLogx()
            gPad.SetLogz()
        ...
        cscatter.SaveAs(
            self.make_file_path(self.d_valevtdata, "cscatter", "eps", None, None)
        )

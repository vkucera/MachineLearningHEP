...
from root_numpy import (  # pylint: disable=import-error, no-name-in-module
    evaluate,
    fill_hist,
)
...
from machine_learning_hep.validation.validation_candidates import (
    fill_validation_candidates,
)
from machine_learning_hep.validation.validation_multiplicity import (
    fill_validation_multiplicity,
)
from machine_learning_hep.validation.validation_vertex import fill_validation_vertex


class ProcesserDhadrons_mult(
    Processer
    ...
    def __init__(
        ...
        self.mc_cut_on_binning2 = datap["analysis"][self.typean].get(
            "mc_cut_on_binning2", True
        )
        ...
        self.event_cand_validation = datap["analysis"][self.typean].get(
            "event_cand_validation", ""
        )
        if "event_cand_validation" not in datap["analysis"][self.typean]:
            self.event_cand_validation = False
        self.usetriggcorrfunc = datap["analysis"][self.typean]["triggersel"].get(
            "usetriggcorrfunc", None
        )
        ...
        self.event_weighting_mc = datap["analysis"][self.typean].get(
            "event_weighting_mc", {}
        )
    ...
    def gethistonormforselevt_mult(
        self, df_evt, dfevtevtsel, label, var, useweightfromfunc=None
    ):
        ...
        if useweightfromfunc is not None:
            weightssel = self.make_weights(
                dfevtevtsel[var], self.weightfunc, self.weighthist, useweightfromfunc
            )
            weightsnovtx = self.make_weights(
                df_no_vtx[var], self.weightfunc, self.weighthist, useweightfromfunc
            )
            weightsgr10 = self.make_weights(
                df_bit_zvtx_gr10[var],
                self.weightfunc,
                self.weighthist,
                useweightfromfunc,
            )

            weightsinvsel = [1.0 / weight for weight in weightssel]
            fill_hist(hSelMult, dfevtevtsel[var], weights=weightsinvsel)
            weightsinvnovtx = [1.0 / weight for weight in weightsnovtx]
            fill_hist(hNoVtxMult, df_no_vtx[var], weights=weightsinvnovtx)
            weightsinvgr10 = [1.0 / weight for weight in weightsgr10]
    ...
    def process_histomass_single(self, index):
        ...
        myfile.cd()
        hsel, hnovtxmult, hvtxoutmult = self.gethistonormforselevt_mult(
            dfevtorig, dfevtevtsel, labeltrigger, self.v_var2_binning_gen
        )
        ...
        if self.usetriggcorrfunc is not None and self.mcordata == "data":
            (
                ...
                hnovtxmultweight,
                hvtxoutmultweight,
            ) = self.gethistonormforselevt_mult(
                dfevtorig,
                dfevtevtsel,
                labeltrigger,
                self.v_var2_binning_gen,
        ...
        for ibin2 in range(len(self.lvar2_binmin)):
            if self.do_inel0[ibin2]:
                dfevtevtsel_inel0 = seldf_singlevar_inclusive(
                    dfevtevtsel,
                    self.v_var2_binning_gen,
                    self.lvar2_binmin[ibin2],
                    self.lvar2_binmax[ibin2],
                )
                dfevtorig_inel0 = seldf_singlevar_inclusive(
                    dfevtorig,
                    self.v_var2_binning_gen,
                    self.lvar2_binmin[ibin2],
                    self.lvar2_binmax[ibin2],
                ...
                if self.usetriggcorrfunc is not None and self.mcordata == "data":
                    (
                        hselweight_inel0,
                        hnovtxmultweight_inel0,
                        hvtxoutmultweight_inel0,
                    ) = self.gethistonormforselevt_mult(
                        dfevtorig_inel0,
                        dfevtevtsel_inel0,
                        labeltrigger_inel0,
        ...
        for ipt in range(self.p_nptfinbins):  # pylint: disable=too-many-nested-blocks
            ...
            if self.runlistrigger is not None:
                df = selectdfrunlist(
                    df, self.run_param[self.runlistrigger], "run_number"
                )
            ...
            for ibin2 in range(len(self.lvar2_binmin)):
                ...
                if self.mltype == "MultiClassification":
                    ...
                    lpt_probcutfin_temp = (
                        1000 * self.lpt_probcutfin[ipt][0] + self.lpt_probcutfin[ipt][1]
                    )
                else:
                ...
                h_invmass_weight = TH1F(
                    "h_invmass_weight" + suffix,
                    "",
                    self.p_num_bins,
                    self.p_mass_fit_lim[0],
                    self.p_mass_fit_lim[1],
                ...
                if self.usetriggcorrfunc is not None and self.mcordata == "data":
                    ...
                    weightsinv = [1.0 / weight for weight in weights]
        ...
        if self.event_cand_validation is True:
            ...
            df_recodtrig = df_recodtrig.query(
                "inv_mass>%f and inv_mass<%f" % (self.mass - 0.15, self.mass + 0.15)
            )
            dfevtwithd = pd.merge(dfevtevtsel, df_recodtrig, on=self.v_evtmatch)
            label = "h%s" % self.v_var2_binning_gen
            histomult = TH1F(
                label, label, self.nbinshisto, self.minvaluehisto, self.maxvaluehisto
            )
    ...
    def get_reweighted_count(self, dfsel, ibin=None):
        ...
        event_weighting_mc = {}
        if (
            self.event_weighting_mc
            and ibin is not None
        ...
        filepath = event_weighting_mc.get(
            "filepath", os.path.join(self.d_mcreweights, self.n_mcreweights)
        )
        if not os.path.exists(filepath):
            print(
                f"Could not find filepath {filepath} for MC event weighting."
        ...
        if not weights:
            print(
                f"Could not find histogram {histo_name} for MC event weighting."
                "Compute unweighted values..."
            )
            return no_weights(dfsel)

        weight_according_to = event_weighting_mc.get(
            "according_to", self.v_var2_binning_gen
        )

        w = [
            weights.GetBinContent(weights.FindBin(v))
            for v in dfsel[weight_according_to]
        ]
        val = sum(w)
    ...
    def process_efficiency_single(self, index):
        ...
        for ibin2 in range(len(self.lvar2_binmin)):
            ...
            h_presel_pr = make_histo(
                "h_presel_pr", "Prompt Reco in acc |#eta|<0.8 and sel"
            )
            h_presel_pr_wotof = make_histo(
                "h_presel_pr_wotof", "Prompt Reco in acc woTOF |#eta|<0.8 and pre-sel"
            )
            h_presel_pr_wtof = make_histo(
                "h_presel_pr_wtof", "Prompt Reco in acc wTOF |#eta|<0.8 and pre-sel"
            )
            h_sel_pr = make_histo(
                "h_sel_pr", "Prompt Reco and sel in acc |#eta|<0.8 and sel"
            )
            h_sel_pr_wotof = make_histo(
                "h_sel_pr_wotof", "Prompt Reco and sel woTOF in acc |#eta|<0.8"
            )
            h_sel_pr_wtof = make_histo(
                "h_sel_pr_wtof", "Prompt Reco and sel wTOF in acc |#eta|<0.8"
            )
            ...
            h_sel_fd = make_histo(
                "h_sel_fd", "FD Reco and sel in acc |#eta|<0.8 and sel"
            )
            ...
            for ipt in range(self.p_nptfinbins):
                ...
                if "nsigTOF_Pr_0" in df_reco_presel_pr:
                    set_content(
                        df_reco_presel_pr[df_reco_presel_pr.nsigTOF_Pr_0 < -998],
                        h_presel_pr_wotof,
                    )
                    set_content(
                        df_reco_presel_pr[df_reco_presel_pr.nsigTOF_Pr_0 > -998],
                ...
                if "nsigTOF_Pr_0" in df_reco_sel_pr:
                    set_content(
                        df_reco_sel_pr[df_reco_sel_pr.nsigTOF_Pr_0 < -998],
                        h_sel_pr_wotof,
                    )
                    set_content(
                        df_reco_sel_pr[df_reco_sel_pr.nsigTOF_Pr_0 > -998],

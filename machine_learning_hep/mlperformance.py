...
def cross_validation_mse_continuous(
    names_, classifiers_, x_train, y_train, cv_, ncores
):
    df_scores = pd.DataFrame()
    for name, clf in zip(names_, classifiers_):
        if "Keras" in name:
...
def plot_cross_validation_mse(names_, df_scores_, suffix_, folder):
    ...
    for name in names_:
        ...
        mystring = "$\\mu=%8.2f, \\sigma=%8.2f$" % (
            df_scores_[name].mean(),
            df_scores_[name].std(),
        )
        plt.text(0.2, 4.0, mystring, fontsize=16)
...
def plotdistributiontarget(names_, testset, myvariablesy, suffix_, folder):
    ...
    for name in names_:
        ...
        plt.hist(
            testset[myvariablesy].values, color="blue", bins=100, label="true value"
        )
        plt.hist(
...
def confusion(names_, classifiers_, suffix_, x_train, y_train, cvgen, folder):
    ...
    plt.subplots_adjust(
        left=None, bottom=None, right=None, top=None, wspace=0.4, hspace=0.2
    )
...
def precision_recall(names_, classifiers_, suffix_, x_train, y_train, cvgen, folder):
    ...
    for name, clf in zip(names_, classifiers_):
        ...
        y_proba = cross_val_predict(
            clf, x_train, y_train, cv=cvgen, method="predict_proba"
        )
        y_scores = y_proba[:, 1]
        ...
        plt.plot(
            thresholds, recalls[:-1], "g-", label="Recall=TP/(TP+FN)", linewidth=5.0
        )
    ...
    for name, clf in zip(names_, classifiers_):
        ...
        plt.plot(
            ...
            tpr,
            "b-",
            label="ROC %s (AUC = %0.2f)" % (name, roc_auc),
            linewidth=5.0,
        )
...
def plot_learning_curves(
    names_, classifiers_, suffix_, folder, x_data, y_data, npoints
):
    ...
    for name, clf in zip(names_, classifiers_):
        ...
        plt.plot(
            arrayvalues, np.sqrt(train_errors), "r-+", linewidth=5, label="training"
        )
...
def plot_overtraining(
    names, classifiers, suffix, folder, x_train, y_train, x_val, y_val, bins=50
):
...
def roc_train_test(
    names, classifiers, x_train, y_train, x_test, y_test, suffix, folder
):

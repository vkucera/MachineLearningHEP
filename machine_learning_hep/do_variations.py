...
def format_value(old, new):
    ...
    if type(old) is type(new):
        if isinstance(old, list):
            ...
            if len_old != len_new:
                msg_warn(
                    "Change of number of elements: %d -> %d\n\t%s -> %s"
...
def format_varlabel(varlabel: list, index: int, n_var: int):
    ...
    return (
        "%s: %d" % (varlabel[0], index) if len(varlabel) != n_var else varlabel[index]
    )
...
def good_list_length(obj, length: int, name=None):
    ...
    if isinstance(obj, dict):
        ...
        msg_err(
            "Object%s is neither a dictionary nor a list."
            % (" %s" % name if name else "")
        )
        result = False
    return result
        ...
        for var in dic_vars:
            ...
            for key in ["activate", "label", "diffs"]:
                if key not in dic_var_single:
                    msg_err(
                        'Key "%s" not found in variation group %s/%s.' % (key, cat, var)
                    )
                    good = False
            if not good:
                return False
            ...
            for i, act in enumerate(dic_var_single["activate"]):
                if not isinstance(act, bool):
                    msg_err(
                        'Element %d of "activate" in %s/%s is not a boolean.'
                        % (i, cat, var)
                    )
                    return False
            ...
            for i, lab in enumerate(dic_var_single["label"]):
                if not isinstance(lab, str):
                    msg_err(
                        'Element %d of "label" in %s/%s is not a string.'
                        % (i, cat, var)
                    )
                    return False
            # Diffs
            ...
            if not good_list_length(dic_var_single["diffs"], length, "diffs"):
                msg_err(
                    '"diffs" in %s/%s does not contain lists of correct length (%d).'
                    % (cat, var, length)
                )
                return False
    return True
...
def main(
    yaml_in, yaml_diff, analysis, clean, proc
    ...
    if proc is not None:
        msg_warn(
            "Only categories that DO%s require running the processor will be processed."
            % ("" if proc else " NOT")
        )
    dic_cats = dic_diff["categories"]
    ...
    for cat in dic_cats:
        ...
        for var in dic_vars:
            ...
            for index in range(n_var):
                ...
                if not dic_var_single["activate"][index]:
                    print(
                        ...
                        % (
                            varstring,
                            label_cat,
                            format_varlabel(label_var, index, n_var),
                        )
                    )
                    continue
                print(
                ...
                # Modify the database.
                if not modify_paths(
                    dic_new, "default/default", varstring, do_processor
                ):
                    sys.exit(1)
                ...
                yaml_out = (
                    yaml_in[:i_dot]
                    + "_%s_%s" % (cat, format_varname(var, index, n_var))
                    + yaml_in[i_dot:]
                )
                ...
                # Start the analysis.
                if analysis:
                    if do_processor and not delete_output_dirs(
                        dic_new, analysis, varstring
                    ):
                        sys.exit(1)
...
if __name__ == "__main__":
    PARSER = argparse.ArgumentParser(
        description="Run the analysis with " "variations of parameters."
    )

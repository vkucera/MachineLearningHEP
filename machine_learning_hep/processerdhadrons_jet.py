...
def special_cuts(row):  # FIXME pylint: disable=fixme
    ...
    normd0Cut = 0.5
    for d0norm_prong in (
        row["imp_par_prong0"] / row["imp_par_err_prong0"],
...
def special_cuts_np(
    ...
    arr_imp_par_prong0,
    arr_imp_par_prong1,
    arr_imp_par_err_prong0,
    arr_imp_par_err_prong1,
    arr_d_len,
    arr_norm_dl,
):
    """ Re-implementation of AliRDHFCutsD0toKpi::IsSelectedSpecialCuts """
    ...
    sel_tot = np.logical_and(
        sel_d0norm_prong_0,
        np.logical_and(sel_d0norm_prong_1, np.logical_and(sel_d_len, sel_norm_dlen)),
    )
    return sel_tot
...
def apply_cut_special_np(df_):
    df_["is_sel_special"] = special_cuts_np(
        df_["pt_cand"],
        df_["eta_cand"],
        df_["imp_par_prong0"],
        df_["imp_par_prong1"],
        df_["imp_par_err_prong0"],
        df_["imp_par_err_prong1"],
        df_["d_len"],
        df_["norm_dl"],
    )
    df_sel = df_[df_["is_sel_special"] == 1]
...
class ProcesserDhadrons_jet(
    ...
    def __init__(
        ...
        self.v_var2_binning_gen = datap["analysis"][self.typean][
            "var_binning2_gen"
        ]  # name
        ...
        self.p_usejetptbinned_deff = datap["analysis"][self.typean].get(
    ...
    def process_histomass_single(self, index):
        ...
        for ipt in range(self.p_nptfinbins):
            ...
            df = seldf_singlevar(
                df, self.v_var_binning, self.lpt_finbinmin[ipt], self.lpt_finbinmax[ipt]
            )
            for ibin2 in range(self.p_nbin2_reco):
                ...
                if self.runlistrigger is not None:
                    df_bin = selectdfrunlist(
                        df_bin, self.run_param[self.runlistrigger], "run_number"
                    )
                ...
                massarray = [
                    1.0 + i * (5.0 / 5000.0) for i in range(5001)
                ]  # 5000 bins in range 1.0-6.0
                ...
                if self.mcordata == "mc":
                    df_bin[self.v_ismcrefl] = np.array(
                        tag_bit_df(df_bin, self.v_bitvar, self.b_mcrefl), dtype=int
                    )
    ...
    def process_efficiency_single(self, index):
        ...
        h2_ptcand_ptjet_genmatched_overflow = build2dhisto(
            "h2_ptcand_ptjet_genmatched_overflow",
            self.var1binarray,
        ...
        h3_shape_ptjet_ptcand_gen = buildhisto(
            "h3_shape_ptjet_ptcand_gen",
            "h3_shape_ptjet_ptcand_gen",
        ...
        for ibinshape in range(self.p_nbinshape_gen):
            for ibin2 in range(self.p_nbin2_gen):
                for ipt in range(self.p_nptfinbins):
                    ...
                    if self.runlistrigger is not None:
                        df_mc_gen = selectdfrunlist(
                            df_mc_gen, self.run_param[self.runlistrigger], "run_number"
                        )
        ...
        for ibin2 in range(
            ...
            for ipt in range(self.p_nptfinbins):
                ...
                if self.runlistrigger is not None:
                    df_mc_reco = selectdfrunlist(
                        df_mc_reco, self.run_param[self.runlistrigger], "run_number"
                    )
                df_mc_gen = pickle.load(
                    openfile(self.mptfiles_gensk[bin_id][index], "rb")
                )
                ...
                df_reco_sel_pr_no_overflow = seldf_singlevar(
                    df_reco_sel_pr,
                    self.v_varshape_binning,
        ...
        for ibin2 in range(self.p_nbin2_gen):
            for ipt in range(self.p_nptfinbins):
                ...
                df_mc_reco["z_gen"] = z_gen_calc(
                    df_mc_reco.pt_gen_jet,
                    df_mc_reco.phi_gen_jet,
                    df_mc_reco.eta_gen_jet,
                    df_mc_reco.pt_gen_cand,
                    df_mc_reco.delta_phi_gen_jet,
                    df_mc_reco.delta_eta_gen_jet,
                )
                df_mc_reco = adjust_z(df_mc_reco)

                # restrict gen shape range
                df_reco_no_overflow = seldf_singlevar(
                    df_mc_reco,
                    self.v_varshape_binning_gen,
    ...
    def create_df_closure(self, df_):
        ...
        df_tmp_selreco = seldf_singlevar(
            df_tmp_selreco,
            "pt_jet",
        ...
        df_tmp_selrecogen = seldf_singlevar(
            df_tmp_selrecogen,
            "pt_jet",
            self.lvar2_binmin_reco[0],
            self.lvar2_binmax_reco[-1],
    ...
    def process_response(self):
        ...
        print(
            "Using run selection for resp histo",
            self.runlistrigger,
            "for period",
            self.period,
        )
    ...
    def process_response_single(self, index):  # pylint: disable=too-many-locals
        ...
        for iptskim in range(self.p_nptbins):
            ...
            if self.doml is True:
                ...
                list_df_mc_reco_ipt = []
                for ipt in range(self.p_nptfinbins):
                    df_mc_reco_ipt = seldf_singlevar(
                        df_mc_reco,
                        self.v_var_binning,
                        self.lpt_finbinmin[ipt],
                        self.lpt_finbinmax[ipt],
        ...
        hzvsjetpt_gen_unmatched = TH2F(
            "hzvsjetpt_gen_unmatched",
            "hzvsjetpt_gen_unmatched",
        ...
        hzvsjetptvscandpt_gen_nonprompt = makefill3dhist(
            df_gen_nonprompt,
            titlehist,
            self.varshapebinarray_gen,
            self.var2binarray_gen,
        ...
        hzvsjetptvscandpt_gen_prompt = makefill3dhist(
            df_gen_prompt,
            titlehist,
            self.varshapebinarray_gen,
            self.var2binarray_gen,
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            hz_gen_nocuts = TH1F(
                "hz_gen_nocuts_nonprompt" + suffix,
                "hz_gen_nocuts_nonprompt" + suffix,
                self.p_nbinshape_gen,
                self.varshapebinarray_gen,
            )
            ...
            df_tmp = seldf_singlevar(
                df_mc_reco_merged_nonprompt,
                "pt_gen_jet",
                self.lvar2_binmin_gen[ibin2],
                self.lvar2_binmax_gen[ibin2],
            )
            ...
            df_tmp = seldf_singlevar(
                df_tmp, "pt_jet", self.lvar2_binmin_reco[0], self.lvar2_binmax_reco[-1]
            )
            df_tmp = seldf_singlevar(
                df_tmp,
                self.v_varshape_binning,
            ...
            hz_gen_nocuts_pr = TH1F(
                "hz_gen_nocuts" + suffix,
                "hz_gen_nocuts" + suffix,
                self.p_nbinshape_gen,
                self.varshapebinarray_gen,
            )
            ...
            df_tmp_pr = seldf_singlevar(
                df_mc_reco_merged_prompt,
                "pt_gen_jet",
                self.lvar2_binmin_gen[ibin2],
                self.lvar2_binmax_gen[ibin2],
            )
        ...
        df_tmp_selgen, df_tmp_selreco, df_tmp_selrecogen = self.create_df_closure(
            df_mc_reco_merged_nonprompt
        )
        ...
        (
            df_tmp_selgen_pr,
            df_tmp_selreco_pr,
            df_tmp_selrecogen_pr,
        ) = self.create_df_closure(df_mc_reco_merged_prompt)

        # histograms for response of feeddown
        hzvsjetpt_reco_nocuts = build2dhisto(
            "hzvsjetpt_reco_nocuts_nonprompt",
            self.varshapebinarray_reco,
            self.var2binarray_reco,
        )
        hzvsjetpt_reco_cuts = build2dhisto(
            "hzvsjetpt_reco_cuts_nonprompt",
            self.varshapebinarray_reco,
            self.var2binarray_reco,
        )
        hzvsjetpt_gen_nocuts = build2dhisto(
            "hzvsjetpt_gen_nocuts_nonprompt",
            self.varshapebinarray_gen,
            self.var2binarray_gen,
        )
        hzvsjetpt_gen_cuts = build2dhisto(
            "hzvsjetpt_gen_cuts_nonprompt",
            self.varshapebinarray_gen,
            self.var2binarray_gen,
        )
        ...
        fill2dhist(
            df_tmp_selreco, hzvsjetpt_reco_nocuts, self.v_varshape_binning, "pt_jet"
        )
        fill2dhist(
            df_tmp_selgen,
        ...
        fill2dhist(
            df_tmp_selrecogen, hzvsjetpt_reco_cuts, self.v_varshape_binning, "pt_jet"
        )
        fill2dhist(
            df_tmp_selrecogen,
            hzvsjetpt_gen_cuts,
        ...
        hzvsjetpt_reco_nocuts_pr = build2dhisto(
            "hzvsjetpt_reco_nocuts", self.varshapebinarray_reco, self.var2binarray_reco
        )
        hzvsjetpt_reco_cuts_pr = build2dhisto(
            "hzvsjetpt_reco_cuts", self.varshapebinarray_reco, self.var2binarray_reco
        )
        hzvsjetpt_gen_nocuts_pr = build2dhisto(
            "hzvsjetpt_gen_nocuts", self.varshapebinarray_gen, self.var2binarray_gen
        )
        hzvsjetpt_gen_cuts_pr = build2dhisto(
            "hzvsjetpt_gen_cuts", self.varshapebinarray_gen, self.var2binarray_gen
        )
        ...
        hzvsjetpt_reco_closure_pr = build2dhisto(
            "hzvsjetpt_reco_closure", self.varshapebinarray_reco, self.var2binarray_reco
        )
        hzvsjetpt_gen_closure_pr = build2dhisto(
            "hzvsjetpt_gen_closure", self.varshapebinarray_reco, self.var2binarray_reco
        )  # FIXME use varshapebinarray_gen pylint: disable=fixme
        hzvsjetpt_reco_pr = build2dhisto(
            "hzvsjetpt_reco", self.varshapebinarray_reco, self.var2binarray_reco
        )
        hzvsjetpt_gen_pr = build2dhisto(
            "hzvsjetpt_gen", self.varshapebinarray_gen, self.var2binarray_gen
        )
        ...
        fill2dhist(
            df_tmp_selreco_pr, hzvsjetpt_reco_pr, self.v_varshape_binning, "pt_jet"
        )
        fill2dhist(
            df_tmp_selgen_pr,
            hzvsjetpt_gen_pr,
        ...
        hjetpt_gen_nocuts_pr = TH1F(
            "hjetpt_gen_nocuts",
            "hjetpt_gen_nocuts",
            self.p_nbin2_gen,
            self.var2binarray_gen,
        )
        hjetpt_gen_cuts_pr = TH1F(
            "hjetpt_gen_cuts",
            "hjetpt_gen_cuts",
            self.p_nbin2_gen,
            self.var2binarray_gen,
        )
        hjetpt_gen_nocuts_closure = TH1F(
            "hjetpt_gen_nocuts_closure",
            "hjetpt_gen_nocuts_closure",
            self.p_nbin2_gen,
            self.var2binarray_gen,
        )
        hjetpt_gen_cuts_closure = TH1F(
            "hjetpt_gen_cuts_closure",
            "hjetpt_gen_cuts_closure",
            self.p_nbin2_gen,
            self.var2binarray_gen,
        )
        ...
        hjetpt_genvsreco_full = TH2F(
            "hjetpt_genvsreco_full_nonprompt",
            "hjetpt_genvsreco_full_nonprompt",
            self.p_nbin2_gen * 100,
            self.lvar2_binmin_gen[0],
            self.lvar2_binmax_gen[-1],
            self.p_nbin2_reco * 100,
            self.lvar2_binmin_reco[0],
            self.lvar2_binmax_reco[-1],
        )

        hjetpt_genvsreco_full_real = buildhisto(
        ...
        hz_genvsreco_full = TH2F(
            "hz_genvsreco_full_nonprompt",
            "hz_genvsreco_full_nonprompt",
            self.p_nbinshape_gen * 100,
            self.lvarshape_binmin_gen[0],
            self.lvarshape_binmax_gen[-1],
            self.p_nbinshape_reco * 100,
            self.lvarshape_binmin_reco[0],
            self.lvarshape_binmax_reco[-1],
        )

        hz_genvsreco_full_real = buildhisto(
        ...
        fill2dhist(
            df_tmp_selrecogen, hjetpt_genvsreco_full_real, "pt_gen_jet", "pt_jet"
        )
        ...
        for row in df_tmp_selrecogen.itertuples():
            response_matrix.Fill(
                getattr(row, self.v_varshape_binning),
                row.pt_jet,
        ...
        hjetpt_genvsreco_full_pr_real = buildhisto(
        ...
        hz_genvsreco_full_pr_real = buildhisto(
            "hz_genvsreco_full_real",
            "hz_genvsreco_full_real",
            self.varshapebinarray_gen,
            self.varshapebinarray_reco,
        ...
        fill2dhist(
            df_tmp_selrecogen_pr, hjetpt_genvsreco_full_pr, "pt_gen_jet", "pt_jet"
        )
        ...
        fill2dhist(
            df_tmp_selrecogen_pr, hjetpt_genvsreco_full_pr_real, "pt_gen_jet", "pt_jet"
        )
        ...
        hzvsjetpt_prior_weights = build2dhisto(
            "hzvsjetpt_prior_weights", self.varshapebinarray_gen, self.var2binarray_gen
        )
        fill2dhist(
            df_tmp_selrecogen_pr,
            hzvsjetpt_prior_weights,
            self.v_varshape_binning_gen,
            "pt_gen_jet",
        )
        # end of histograms for unfolding
        ...
        for ibin2 in range(self.p_nbin2_reco):
            df_tmp_selrecogen_jetbin = seldf_singlevar(
                df_tmp_selrecogen,
                "pt_jet",
                self.lvar2_binmin_reco[ibin2],
                self.lvar2_binmax_reco[ibin2],
            ...
            df_tmp_selrecogen_pr_jetbin = seldf_singlevar(
                df_tmp_selrecogen_pr,
                "pt_jet",
                self.lvar2_binmin_reco[ibin2],
                self.lvar2_binmax_reco[ibin2],
        ...
        for ibinshape in range(len(self.lvarshape_binmin_reco)):
            df_tmp_selrecogen_zbin = seldf_singlevar(
                df_tmp_selrecogen,
                self.v_varshape_binning,
            ...
            df_tmp_selrecogen_pr_zbin = seldf_singlevar(
                df_tmp_selrecogen_pr,
                self.v_varshape_binning,
                self.lvarshape_binmin_reco[ibinshape],
                self.lvarshape_binmax_reco[ibinshape],
            ...
            hjetpt_genvsreco_pr = TH2F(
                "hjetpt_genvsreco" + suffix,
                "hjetpt_genvsreco" + suffix,
                self.p_nbin2_gen * 100,
                self.lvar2_binmin_gen[0],
                self.lvar2_binmax_gen[-1],
            ...
            fill2dhist(
                df_tmp_selrecogen_pr_zbin, hjetpt_genvsreco_pr, "pt_gen_jet", "pt_jet"
            )
        ...
        for ibinshape in range(len(self.lvarshape_binmin_gen)):
            dtmp_nonprompt_zgen = seldf_singlevar(
                df_mc_reco_merged_nonprompt,
                self.v_varshape_binning_gen,
                self.lvarshape_binmin_gen[ibinshape],
                self.lvarshape_binmax_gen[ibinshape],
            ...
            fill_hist(
                hz_fracdiff,
                (
                    dtmp_nonprompt_zgen[self.v_varshape_binning]
                    - dtmp_nonprompt_zgen[self.v_varshape_binning_gen]
                )
                / dtmp_nonprompt_zgen[self.v_varshape_binning_gen],
            )
            ...
            dtmp_prompt_zgen = seldf_singlevar(
                df_mc_reco_merged_prompt,
                self.v_varshape_binning_gen,
                self.lvarshape_binmin_gen[ibinshape],
                self.lvarshape_binmax_gen[ibinshape],
            ...
            hz_fracdiff_pr = TH1F(
                "hz_fracdiff_prompt" + suffix, "hz_fracdiff_prompt" + suffix, 100, -2, 2
            )
            fill_hist(
                hz_fracdiff_pr,
                (
                    dtmp_prompt_zgen[self.v_varshape_binning]
                    - dtmp_prompt_zgen[self.v_varshape_binning_gen]
                )
                / dtmp_prompt_zgen[self.v_varshape_binning_gen],
            )
        ...
        for ibin2 in range(self.p_nbin2_gen):
            dtmp_nonprompt_jetptgen = seldf_singlevar(
                df_mc_reco_merged_nonprompt,
                "pt_gen_jet",
                self.lvar2_binmin_gen[ibin2],
                self.lvar2_binmax_gen[ibin2],
            ...
            hjetpt_fracdiff = TH1F(
                "hjetpt_fracdiff_nonprompt" + suffix,
                "hjetpt_fracdiff_nonprompt" + suffix,
                100,
                -2,
                2,
            ...
            fill_hist(
                hjetpt_fracdiff,
                (
                    dtmp_nonprompt_jetptgen["pt_jet"]
                    - dtmp_nonprompt_jetptgen["pt_gen_jet"]
                )
                / dtmp_nonprompt_jetptgen["pt_gen_jet"],
            )
            ...
            dtmp_prompt_jetptgen = seldf_singlevar(
                df_mc_reco_merged_prompt,
                "pt_gen_jet",
                self.lvar2_binmin_gen[ibin2],
                self.lvar2_binmax_gen[ibin2],
            ...
            hjetpt_fracdiff_pr = TH1F(
                "hjetpt_fracdiff_prompt" + suffix,
                "hjetpt_fracdiff_prompt" + suffix,
                100,
                -2,
                2,
            ...
            fill_hist(
                hjetpt_fracdiff_pr,
                (dtmp_prompt_jetptgen["pt_jet"] - dtmp_prompt_jetptgen["pt_gen_jet"])
                / dtmp_prompt_jetptgen["pt_gen_jet"],
            )
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            df_tmp_selgen_pr_test_bin = seldf_singlevar(
                df_tmp_selgen_pr_test,
                "pt_gen_jet",
                self.lvar2_binmin_gen[ibin2],
                self.lvar2_binmax_gen[ibin2],
            )
            df_tmp_selrecogen_pr_test_bin = seldf_singlevar(
                df_tmp_selrecogen_pr_test,
                "pt_gen_jet",
                self.lvar2_binmin_gen[ibin2],
                self.lvar2_binmax_gen[ibin2],
            ...
            fill_hist(
                hz_gen_nocuts_closure,
                df_tmp_selgen_pr_test_bin[self.v_varshape_binning_gen],
            )
            fill_hist(
                hz_gen_cuts_closure,
                df_tmp_selrecogen_pr_test_bin[self.v_varshape_binning_gen],
            )
        ...
        hzvsjetpt_reco_nocuts_closure = TH2F(
        ...
        hzvsjetpt_reco_cuts_closure = TH2F(
        ...
        for row in df_tmp_selrecogen_pr.itertuples():
            ...
            response_matrix_pr.Fill(
                getattr(row, self.v_varshape_binning),
                row.pt_jet,
                getattr(row, self.v_varshape_binning_gen),
                row.pt_gen_jet,
                response_matrix_weight,
            )
        for row in df_tmp_selrecogen_pr_train.itertuples():
            ...
            if self.doprior is True:
                ...
                if weight > 0.0:
                    response_matrix_weight = 1.0 / weight
            response_matrix_closure_pr.Fill(
                getattr(row, self.v_varshape_binning),
                row.pt_jet,
                getattr(row, self.v_varshape_binning_gen),
                row.pt_gen_jet,

...
def selectpid_dstokkpi(
    ...
    for icand, _ in enumerate(array_nsigma_tpc_pi_0):
        ...
        is_track_1_sel = (
            array_nsigma_tpc_k_1[icand] < nsigmacut
...
def selectpid_lctov0bachelor(array_nsigma_tpc, array_nsigma_tof, nsigmacut):
    ...
    for icand, _ in enumerate(array_nsigma_tpc):
        is_track_sel = (
            array_nsigma_tpc[icand] < nsigmacut or array_nsigma_tof[icand] < nsigmacut
        )
        if is_track_sel:
            array_is_pid_sel.append(True)
        else:
...
def getnormforselevt(df_evt):
    ...
    return (n_ev_sel + n_no_reco_vtx) - n_no_reco_vtx * n_zvtx_gr10 / (
        n_ev_sel + n_zvtx_gr10
    )

...
graphpbpb05_sym = TGraphErrors(
    len(etaval_list_o), etaval_list_d, dndeta_list_d, erretaval_list_d, errdndeta_list_d
)
...
for i, species in enumerate(speciesrun5):
    ...
    scalefactor = (
        npartrun5[i] * 0.5 * dndetaperpairrun5[i] / listdndetafitrun5[i].Eval(0.0)
    )
    listdndetafitrun5[i].FixParameter(
        0, listdndetafitrun5[i].GetParameter(0) * scalefactor
    )
    listdndetafitrun5[i].FixParameter(
        1, listdndetafitrun5[i].GetParameter(1) * scalefactor
    )
    listdndetafitrun5[i].FixParameter(
        2, listdndetafitrun5[i].GetParameter(2) * scalefactor
    )
    listdndetafitrun5[i].FixParameter(
        3, listdndetafitrun5[i].GetParameter(3) * scalefactor
    )
    listdndetafitrun5[i].FixParameter(
        6, listdndetafitrun5[i].GetParameter(6) * scalefactor
    )
    ...
    for index, etap in enumerate(dndeta_points):
        ...
        print(
            "dndeta at eta=%f" % etap + ", val =%.2f" % listdndetafitrun5[i].Eval(etap)
        )
    print(
        "dndeta at -4<eta<4, val =%.2f"
...
for i, species in enumerate(speciesrun5):
    ...
    leg.AddEntry(
        listdndetafitrun5[i],
        "Extrapolation %s at %.2f TeV" % (speciesrun5[i], energyrun5[i]),
    )

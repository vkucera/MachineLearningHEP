...
def vardistplot(
    dataframe_sig_, dataframe_bkg_, mylistvariables_, output_, binmin, binmax
):
...
def vardistplot_probscan(
    ...
    for thresh_index, threshold in enumerate(thresharray_):
        ...
        for i, var in enumerate(mylistvariables_):
            ...
            his, bina = np.histogram(
                values, range=(xrange_min[i], xrange_max[i]), bins=100
            )
            if thresh_index == 0:
            ...
            axes[i].bar(
                center, his, align="center", width=width, facecolor=clr, label=lbl
            )
...
def efficiency_cutscan(
    ...
    for i, var_tuple in enumerate(mylistvariables_):
        ...
        if "abs" in vardir:
            ...
            if cen is None:
                get_logger().error(
                    "Absolute cut chosen for %s. " "However, no central value provided",
                    var,
                )
                continue
        ...
        if vardir not in cut_options:
            get_logger().error(
                "Please choose cut option from %s. "
                "Your current setting for variable %s is %s",
                str(cut_options),
                vardir,
                var,
            )
...
def picklesize_cutscan(
    dataframe_, mylistvariables_, output_, suffix_, plot_options_=None
...
def scatterplot(
    ...
    for j, _ in enumerate(mylistvariablesx_):
        ...
        plt.scatter(
            dataframe_bkg_[mylistvariablesx_[j]],
            dataframe_bkg_[mylistvariablesy_[j]],
            alpha=0.4,
            c="g",
            label="background",
        )
        plt.scatter(
            dataframe_sig_[mylistvariablesx_[j]],
            dataframe_sig_[mylistvariablesy_[j]],
            alpha=0.4,
            c="b",
            label="signal",
        )
        plt.title(
            "Pearson sgn: %s"
            % dataframe_sig_.corr()
            .loc[mylistvariablesx_[j]][mylistvariablesy_[j]]
            .round(2)
            + ",  Pearson bkg: %s"
            % dataframe_bkg_.corr()
            .loc[mylistvariablesx_[j]][mylistvariablesy_[j]]

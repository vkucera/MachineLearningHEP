...
class KerasClassifierBayesianOpt(
    BayesianOpt
    ...
    def get_scikit_model(self):
        ...
        layer = Dense(
            self.space_tmp["n_nodes"][0], activation=self.space_tmp["activation_0"]
        )(inputs)
        for i, n_nodes in enumerate(self.space_tmp["n_nodes"][1:]):
            layer = Dense(n_nodes, activation=self.space_tmp[f"activation_{(i+1)%2}"])(
                layer
            )
        predictions = Dense(1, activation="sigmoid")(layer)
        # Build model from layers
        model = Model(inputs=inputs, outputs=predictions)
        model.compile(
            loss=self.model_config_tmp["loss"],
    ...
    def yield_model_(self, model_config, space):
        ...
        return (
            KerasClassifier(
                build_fn=self.get_scikit_model,
                epochs=space["epochs"],

...
def buildhisto(h_name, h_tit, arrayx, arrayy=None, arrayz=None):
    ...
    if arrayz:
        histo = TH3F(
            h_name, h_tit, *binning(arrayx), *binning(arrayy), *binning(arrayz)
        )
    elif arrayy:
...
def scatterplotroot(dfevt, nvar1, nvar2, nbins1, min1, max1, nbins2, min2, max2):
    ...
    """
    hmult1_mult2 = TH2F(
        nvar1 + nvar2, nvar1 + nvar2, nbins1, min1, max1, nbins2, min2, max2
    )
...
def find_axes_limits(histos, use_log_y=False):
    ...
    for h in histos:
        ...
        if min_y_tmp > 0.0 and use_log_y or not use_log_y:
...
def divide_by_eachother(histos1, histos2, scale=None, rebin2=None):
    ...
    if len(histos1) != len(histos2):
        get_logger().fatal(
            "Number of histograms mismatch, %i vs. %i", len(histos1), len(histos2)
        )
    ...
    for i, _ in enumerate(histos1):
        ...
        if rebin2 is not None:
            ...
            histos1[i] = histos1[i].Rebin(
                len(rebin2) - 1, f"{histos1[i].GetName()}_rebin", rebin
            )
            histos2[i] = histos2[i].Rebin(
                len(rebin2) - 1, f"{histos2[i].GetName()}_rebin", rebin
            )
...
def divide_by_eachother_barlow(histos1, histos2, scale=None, rebin2=None):
    ...
    for i, _ in enumerate(histos1):
        ...
        for j in range(histos1[i].GetNbinsX()):
            stat1.append(
                histos1[i].GetBinError(j + 1) / histos1[i].GetBinContent(j + 1)
            )
            stat2.append(
                histos2[i].GetBinError(j + 1) / histos2[i].GetBinContent(j + 1)
            )
        ...
        for j in range(histos_ratio[-1].GetNbinsX()):
            ...
            histos_ratio[-1].SetBinError(
                j + 1, histos_ratio[-1].GetBinContent(j + 1) * statunc
            )
...
def put_in_pad(
    pad, use_log_y, histos, title="", x_label="", y_label="", yrange=None, **kwargs
):
    """
    ...
    frame = pad.DrawFrame(
        min_x, yrange[0], max_x, yrange[1], f"{title};{x_label};{y_label}"
    )
...
def plot_histograms(
    ...
    **kwargs,
    ...
    style_histograms(
        histos, linestyles, markerstyles, colors, linewidths, fillstyles, fillcolors
    )
...
def calc_systematic_multovermb(
    errnum_list, errden_list, n_bins, same_mc_used=False, justfd=-99
):
    """
    ...
    if n_bins != len(list(errnum_list.errors.values())[0]) or n_bins != len(
        list(errden_list.errors.values())[0]
    ):
        get_logger().fatal(
    ...
    j = 0
    for (_, errnum), (_, errden) in zip(
        errnum_list.errors.items(), errden_list.errors.items()
    ):
        for i in range(n_bins):
            ...
            if errnum_list.names[j] != errden_list.names[j]:
                get_logger().fatal(
                    "Names not in same order: %s vs %s",
                    errnum_list.names[j],
                    errden_list.names[j],
                )
            ...
            for nb in range(len(tot_list[i])):
                if errnum_list.names[j] == "yield" and justfd is not True:
                    ...
                    tot_list[i][nb] += max(errnum[i][nb], errden[i][nb]) * max(
                        errnum[i][nb], errden[i][nb]
                    )
                    ...
                    errnum_list.names[j] == "feeddown_mult_spectra"
                    and justfd is not False
                ):
                    ...
                    and justfd is not True
                ):
                    # FD: estimated using 7TeV strategy directly for ratio
                    tot_list[i][nb] += errnum[i][nb] * errnum[i][nb]
                elif (
                    errnum_list.names[j] == "multiplicity_weights"
                    and justfd is not True
                ):
                    # Uncorrelated
                    tot_list[i][nb] += (
                        errnum[i][nb] * errnum[i][nb] + errden[i][nb] * errden[i][nb]
                    )
        ...
        for i in range(n_bins):
            ...
            for nb in range(len(tot_list[i])):
                if errnum_list.names[j] == "yield" and justfd is not True:
                    ...
                    if nb == 2:
                        tot_list[i][nb] += (
                            (rat[1] - minsys) * (rat[1] - minsys) / (rat[1] * rat[1])
                        )
                    if nb == 3:
                        tot_list[i][nb] += (
                            (maxsys - rat[1]) * (maxsys - rat[1]) / (rat[1] * rat[1])
                        )
                elif errnum_list.names[j] == "feeddown_mult" and justfd is not False:
...
def calc_systematic_mesondoubleratio(
    ...
    """
    tot_list = [[0.0, 0.0, 0.0, 0.0] for _ in range(n_bins)]
    if n_bins != len(list(errnum_list1.errors.values())[0]) or n_bins != len(
        list(errden_list1.errors.values())[0]
    ):
        if dropbins is None:
            get_logger().fatal(
                "Number of bins and number of errors mismatch, %i vs. %i vs. %i",
        ...
        for i in range(n_bins):
            ...
            for nb in range(len(tot_list[i])):
                if errnum_list1.names[j] == "yield" and justfd is not True:
                    ...
                    errnum_list1.names[j] == "feeddown_mult_spectra"
                    and justfd is not False
                ):
                    # Correlated, do nothing
                    pass
                    ...
                    errnum_list1.names[j] == "multiplicity_weights"
                    and justfd is not True
                ):
                    # Correlated, do nothing
                    pass
                    ...
                    errnum_list1.names[j] == "multiplicity_interval"
                    and justfd is not True
                ):
                    # NB: Assuming ratio: 3prongs over 2prongs here! 2prong part cancels
                    # We use 1/3 of systematic of numerator
...
def average_pkpi_pk0s(
    ...
    if len(matchbins_pkpi) != len(matchbins_pk0s):
        get_logger().fatal(
            "Length matchbins_pkpi != matchbins_pk0s: %d != %d",
            len(matchbins_pkpi),
            len(matchbins_pk0s),
        )
    ...
    for j in range(2):
        for ipt in range(nbins):
            ...
            if binmatch < 0:
                ...
                rel_stat_unc[j][ipt] = arr_histo[j].GetBinError(binmatch) / arr_histo[
                    j
                ].GetBinContent(binmatch)
    ...
    for ipt in range(nbins):
        ...
        for j in range(2):
            ...
            average_statunc[ipt] += (stat_unc[j][ipt] * weight) * (
                stat_unc[j][ipt] * weight
            )
    ...
    (
        ...
        average_fpromptlow,
        average_fprompthigh,
    ) = weight_systematic_lc_averaging(
...
def weight_systematic_lc_averaging(
    arr_errors, fprompt, fpromptlow, fprompthigh, arr_weights, arr_weightsum
):
    """
    ...
    j = 0
    for (_, errpkpi), (_, errpk0s) in zip(
        arr_errors[0].errors.items(), arr_errors[1].errors.items()
    ):
        ...
        for i in range(nbins):
            ...
            if arr_errors[0].names[j] not in listimpl:
                get_logger().fatal(
                    "Unknown systematic name: %s", arr_errors[0].names[j]
                )
            if arr_errors[0].names[j] != arr_errors[1].names[j]:
                get_logger().fatal(
                    "Names not in same order: %s vs %s",
                    arr_errors[0].names[j],
                    arr_errors[1].names[j],
                )
    ...
    for i in range(nbins):
        fpromptlownew[i] = (
            (fpromptlow[0][i] * arr_weights[0][i])
            + (fpromptlow[1][i] * arr_weights[1][i])
        ) / arr_weightsum[i]
        fprompthighnew[i] = (
            (fprompthigh[0][i] * arr_weights[0][i])
...
class Errors:
    ...
    def make_root_asymm(histo_central, error_list, **kwargs):
        ...
        if n_bins != len(error_list):
            get_logger().fatal(
                "Number of bins and number of errors mismatch, %i vs. %i",
                n_bins,
                len(error_list),
            )
        ...
        if const_x_err is not None:
            ...
            x_up = array(
                "d",
                [
                    err[1] * histo_central.GetBinCenter(b + 1)
            ...
            x_low = array(
                "d",
                [
                    err[0] * histo_central.GetBinCenter(b + 1)
                    for b, err in enumerate(error_list)
                ],
            )
        else:
        ...
        if const_y_err is not None:
            ...
            y_up = array(
                "d",
                [
                    err[3] * histo_central.GetBinContent(b + 1)
                    for b, err in enumerate(error_list)
                ],
            )
            y_low = array(
                "d",
                [
                    err[2] * histo_central.GetBinContent(b + 1)
                    for b, err in enumerate(error_list)
                ],
            )
        else:
        ...
        bin_centers = array(
            "d", [histo_central.GetBinCenter(b + 1) for b in range(n_bins)]
        )
        bin_contents = array(
            "d", [histo_central.GetBinContent(b + 1) for b in range(n_bins)]
        )

        return TGraphAsymmErrors(
            n_bins, bin_centers, bin_contents, x_low, x_up, y_low, y_up
        )
    ...
    def add_errors(self, name, err_list):
        ...
        if len(err_list) != self.n_bins:
            self.logger.fatal(
                "%i errors required, you want to push %i", self.n_bins, len(err_list)
            )
                ...
                for nb in range(len(tot_list[i])):
                    ...
                    if self.names[j] != "sigmav0" and self.names[j] != "feeddown_mult":
                        ...
                        if justfd == -99:
                            ...
                            get_logger().fatal(
                                "Option for spectra systematic not valid"
                            )

...
def getclf_scikit(model_config):
    ...
    for c in model_config["scikit"]:
        if model_config["scikit"][c]["activate"]:
            try:
                model = getattr(templates_scikit, c)(
                ...
                grid_search_params.append(
                    model_config["scikit"][c].get("grid_search", {})
                )
...
def getclf_xgboost(model_config):
    ...
    for c in model_config["xgboost"]:
        if model_config["xgboost"][c]["activate"]:
            try:
                model = getattr(templates_xgboost, c)(
                ...
                names.append(c)
                grid_search_params.append(
                    model_config["xgboost"][c].get("grid_search", {})
                )
...
def getclf_keras(model_config, length_input):
    ...
    for c in model_config["keras"]:
        if model_config["keras"][c]["activate"]:
            try:
                model = getattr(templates_keras, c)(
...
def test(
    ...
    y_test_ = test_set_[myvariablesy_].values.reshape(
        len(x_test_),
    )
    ...
    for name, model in zip(names_, trainedmodels_):
        ...
        y_test_prediction = y_test_prediction.reshape(
            len(y_test_prediction),
        )
        test_set_["y_test_prediction" + name] = pd.Series(
            y_test_prediction, index=test_set_.index
        )

        y_test_prob = model.predict_proba(x_test_)
        if ml_type == "BinaryClassification":
            test_set_["y_test_prob" + name] = pd.Series(
                y_test_prob[:, 1], index=test_set_.index
            )
        elif ml_type == "MultiClassification" and labels_ is not None:
            for pred, lab in enumerate(labels_):
                test_set_["y_test_prob" + name + lab] = pd.Series(
                    y_test_prob[:, pred], index=test_set_.index
                )
        else:
...
def apply(
    ml_type, names_, trainedmodels_, test_set_, mylistvariablestraining_, labels_=None
):
    logger = get_logger()
...
def decisionboundaries(names_, trainedmodels_, suffix_, x_train_, y_train_, folder):
    ...
    xx, yy = np.meshgrid(
        np.arange(x_min, x_max, height), np.arange(y_min, y_max, height)
    )

...
def main():
    argp = ArgumentParser(
        description="Download or update input data for MachineLearningHEP"
    )
    argp.add_argument(
        "--verbose",
        dest="verbose",
    ...
    try:
        subprocess.check_call(
            ["curl", "-L", SOURCE, "-o", zipFile], stdout=redir, stderr=redir
        )
    ...
    try:
        subprocess.check_call(
            ["unzip", "-o", "-d", args.dest, zipFile], stdout=redir, stderr=redir
        )
